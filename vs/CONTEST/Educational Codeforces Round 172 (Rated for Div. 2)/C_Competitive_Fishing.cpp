...
bool good(int mid,int one,int k){
    if(mid<=1) return 0;
    else{
        int sum=(mid-1)*one;
        int minus=(mid-1)*(mid-2)/2;
        sum-=minus;
        return sum>=k;
    }
}
...
//     }
//     if(str.empty()){
//         cout<<-1<<endl; // as its impossible to get k(>1)
//     }
//     else{
//         // cout<<str<<endl;
//         int one=0;
//         for(int i=0;i<str.size();i++) if(str[i]=='1') one++;
//         bool haszero=(str[0]=='0');
//         if(!haszero) one--;
//         // cout<<"one: "<<one<<endl;
//         int high=one+2,low=-1;
//         while(high-low>1){
//             int mid=(high+low)/2;
//             if(good(mid,one,k)) high=mid;
//             else low=mid;
//             // cout<<"mid: "<<mid<<endl;
//         }
//         if(low==-1 || high==one+2) cout<<-1<<endl;
//         else cout<<high<<endl;
//     }
// }
