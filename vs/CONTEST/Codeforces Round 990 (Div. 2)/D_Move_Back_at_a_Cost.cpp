...
void solve(int &t,int &T){
    ...
    vector<int>suff(n);
    suff[n-1]=a[n-1];
    multiset<int>noNeed,need;
    noNeed.insert(a[n-1]);
    for(int i=n-2;i>=0;i--){
        suff[i]=min(suff[i+1],a[i]);
        if(suff[i]==a[i]) noNeed.insert(a[i]);
        else need.insert(a[i]);
    }
    vector<int>res;
    bool flag=false;
    while(!noNeed.empty() && !need.empty()){
        if(!flag){
            if(*noNeed.begin()<=*need.begin()+1){
                res.pb(*noNeed.begin());
                noNeed.erase(noNeed.begin());
            }
            else flag=true;
        }
        else{
            if(*noNeed.begin()+1<=*need.begin()+1){
                res.pb(*noNeed.begin()+1);
                noNeed.erase(noNeed.begin());
            }
            else{
                res.pb(*need.begin()+1);
