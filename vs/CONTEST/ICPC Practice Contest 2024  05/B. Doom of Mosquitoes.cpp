...
bool isSame(double x1,double y1,double x2,double y2,double x3,double y3){
    return fabs((x2-x1)*(y3-y1)-(y2-y1)*(x3-x1))<1e-9;
}

int max_mosha(int n,int m,vector<pair<double,double>>&mosha,vector<pair<double,double>>&lasers){
    int max_kill=0;
    for (int i=0;i<m;i++) {
        for (int j=i+1;j<m;j++) {
            double x1=lasers[i].first,y1 =lasers[i].second;
            double x2=lasers[j].first,y2 =lasers[j].second;
            int count=0;
            for (int k=0;k<n;k++) {
                double x3 = mosha[k].first, y3 = mosha[k].second;
...
void solve(int &t,int &T){
    // ll in,n,m,i,j,k,x,y;
    int n,m; cin>>n>>m;
    vector<pair<double,double>>mosha(n);
    vector<pair<double,double>>lasers(m);

    for(int i=0;i<n;i++){
        cin>>mosha[i].first>>mosha[i].second;
    }

    for(int j=0;j<m;j++){
        cin>>lasers[j].first>>lasers[j].second;
    }

    int result=max_mosha(n,m,mosha,lasers);
    cout<<result<<endl;
