...
#define Unset(a,b) (a&=~(1LL<<b))
#define Check(a,b) (a&(1LL<<b))
#define Toggle(a,b) (x^(1LL<<b))
...
int calculatteSteps(int H,vector<int>&S,int sz){
    int count=0;
    for(int i=1;i<(1<<sz);i++){
        int lCm=1,x_or=0;
        for(int j=0;j<sz;j++){
            if(Check(i,j)) lCm=lcm(lCm,S[j]),x_or^=1;
        }
        if(x_or) count+=H/lCm+1;
        else count-=H/lCm+1;
    }
    return count;
}
...
void solve(int &t,int &T){
    ...
    int alice=0;
    for(int i=0;i<n;i++){
        int count=calculatteSteps(H[i],S,sz);
        alice^=count;
    }
    cout<<"Case "<<t<<": ";
