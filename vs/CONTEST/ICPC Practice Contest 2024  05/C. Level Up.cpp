...
void solve(int &t, int &T) {
    ...
    int levelsAfter2=l-b;
    int ways=((b-a)%mod)*((l-b)%mod)%mod;
    int temp1=levelsAfter2;
    int temp2=levelsAfter2-1;
    if (temp1%2==1) temp2/=2; 
    else temp1/=2;  
    levelsAfter2=(temp1%mod*temp2%mod)%mod;
    int ans=(levelsAfter2+ways)%mod;
    l-=3; 
    ans=(bigmod(n-1,l,mod)*ans)%mod;
