...
int countDivisors(map<int,int>& factors){
    int divisors=1;
    for (auto it : factors) {
        int exp=it.second;
...
void solve(int &t,int &T){
    ...
    map<int,int>factorialPrimeExponents;
    for(int i=2;i<=m;i++) {
        auto factors=factorize(i);
        for(auto it:factors){   // prime, exponent
            factorialPrimeExponents[it.first]+=it.second;
        }
    }
    for(int i=0;i<n;i++){
        auto factors=factorize(a[i]);
        for(auto it:factorialPrimeExponents){
            factors[it.first]+=it.second;
        }
        cout<<countDivisors(factors)<<" ";
    }
    cout<<endl;
}
