...
void print(vector<vector<int>>&v,queue<int>q){
    cout<<"v:"<<endl;
...
void friends(int idx,vector<int>&h,vector<vector<int>>&v,queue<int>&q){
    int n=h.size();
    mp[idx]++;
    for(int i=v[idx].back()+1;i<n;i++){
        if(mp[h[idx]]==1) break;
        v[idx].pb(i);
        q.push(i);
    }
    for(int i=1;i<v[idx].size();i++){
        int friendIdx=v[idx][i];
        for(int j=v[friendIdx].back()+1;j<n;j++){
            if(h[friendIdx]==h[j]) break;
            v[friendIdx].pb(j);
            q.push(i);
        }
    }
