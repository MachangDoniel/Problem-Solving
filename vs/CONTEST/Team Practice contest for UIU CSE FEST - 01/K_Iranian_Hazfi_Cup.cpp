...
vector<priority_queue<pair<int,int>>>adj;

bool noBracket(string& s){
...
int getPenaltyScore(string& point){
    bool foundBracket=false;
    string s;
    for(int i=0;i<point.size();i++){
        if(point[i]=='(') foundBracket=true;
        else if(point[i]==')') break;
        else{
            if(foundBracket){
                s+=point[i];
            }
...
bool whoWins(string& point1, string& point2){
    int score1,score2;
    if(noBracket(point1) && noBracket(point2)){
        score1=stoi(point1);
        score2=stoi(point2);
    }
    else{
        score1=getPenaltyScore(point1);
        score2=getPenaltyScore(point2);
    }
    return score1>score2;
}
vector<int>hashvalue;
int getNode(int node){
    int nextnode=adj[node].top().second;
    adj[node].pop();
    adj[nextnode].pop();
    return nextnode;
}
void makeTree(int node,int left,int right){
    if(left==right) hashvalue[node]=left;
    else{
        int mid=(left+right)/2;
        int nextnode=getNode(node);
        makeTree(node,left,mid);
        makeTree(nextnode,mid+1,right);
    }
}

bool ok(int n1,int n2,int k){
    int mid=(1<<k);
    // cout<<"mid: "<<mid<<endl;
    return (n1<=mid && n2<=mid);
}
int res=0;
void search2(int n1,int n2,int left,int right,int k){
    ...
    // 1st segment
    if(n1<=mid && n2<=mid){
        search2(n1,n2,left,mid,k-1);
    }
    // 2nd segment
    else if(n1>mid && n2>mid){
        search2(n1,n2,mid+1,right,k-1);
    }
    else{
        res=k;
...
int getans(int n1,int n2,int k){
    search2(n1,n2,1,(1<<k),k);
    return res;
}
...
void solve(int &t,int &T){
    ...
    int hash=0;
    vector<int>wins(1<<k,0);
    vector<pair<int,int>>matches;
    for(int i=0;i<(1<<k)-1;i++){
        string team1,point1,dash,team2,point2;
        cin>>team1>>point1>>dash>>point2>>team2;
        if(mp.find(team1)==mp.end()) mp[team1]=hash++;
        if(mp.find(team2)==mp.end()) mp[team2]=hash++;
        if(whoWins(point1,point2)){
            // team1 wins
            wins[mp[team1]]++;
        }
        else{
        ...
        matches.pb({mp[team1],mp[team2]});
    }
    int maxwin=0,maxwinner=-1;
    ...
    hashvalue.resize((1<<k));
    makeTree(maxwinner,0,(1<<k)-1);
    // for(int i=0;i<hashvalue.size();i++){
    //     cout<<i<<": "<<hashvalue[i]<<endl;
    // }
    // cout<<endl;
    for(int i=0;i<n;i++){
        string team1,team2;
        cin>>team1>>team2;
        int index1=hashvalue[mp[team1]]+1;
        int index2=hashvalue[mp[team2]]+1;
        // cout<<"index: "<<index1<<" "<<index2<<endl;
        cout<<getans(min(index1,index2),max(index1,index2),k)<<endl;
    }
}
