...
bool yHasAllx(int x,int y){
    map<int,int>Y;
    for(int i=0;i<v[y].second.size();i++) Y[v[y].second[i]]++;
    for(int i=0;i<v[x].second.size();i++) if(Y[v[x].second[i]]==0) return false;
    return true;
}
bool yHasMorex(int x,int y){
    set<int>s;
    for(int i=0;i<v[y].second.size();i++){
        s.insert(v[y].second[i]);
    }
    for(int i=0;i<v[x].second.size();i++){
        s.erase(s.find(v[x].second[i]));
    }
    return !s.empty();
}
...
bool Compare(int x,int y){
    if(v[x].first>=v[y].first){
        if(yHasAllx(x,y)){
            if(v[x].first>v[y].first || yHasMorex(x,y)){
                return true;
            }
            return false;
        }
