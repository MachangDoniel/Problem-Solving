...
main()
{
    ...
    for(int t=1;t<=T;t++){
        ...
        if(a!=c-1) cout<<-1<<endl;
            ...
            int heightBya=log2(a+1),height=0;
            // cout<<heightBya<<" ";
            if(heightBya>0 && power(2,heightBya)!=(a+1)) heightBya++;
            int specialEdgeBya=power(2,heightBya)-(a+1);    // specialEdgeBya is the edge that makes 1 less height than other edges
            // cout<<heightBya<<" "<<specialEdgeBya<<endl;  // edgeBya is the edge that can be produced by a, where c=(b+(c-b)) is the actual edge
            height=heightBya;   // height by a & c
            if(b>=specialEdgeBya) b-=specialEdgeBya;
            else b=0;
            if(b>=c) height+=b/c,b%=c;
            if(b) height++;
            cout<<height<<endl;
        }
