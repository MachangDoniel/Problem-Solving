...
main()
{
    ...
    for(int t=1;t<=T;t++){
        // ll in,n,m,i,j,k,x,y;
        int n,m; cin>>n>>m;
        vector<vector<pll>>a(n,vector<pll>(27,{0,0}));
        for(int i=0;i<n;i++){
            string s; cin>>s;
            int prev=0;
            mpl mp;
            for(int j=0;j<m;j++){
                if(!mp[s[j]-'a'+1]) a[i][s[j]-'a'+1].first=j+1;
                mp[s[j]-'a'+1]++;
                a[i][s[j]-'a'+1].second=max(j+1,a[i][s[j]-'a'+1].second);
            }
        }
        // for(int i=0;i<n;i++){
        //     for(int j=0;j<27;j++){
        ...
        for(int i=0;i<q;i++){
            ...
            for(int i=1;i<27;i++){
                int upper=min(a[x-1][i].second,a[y-1][i].second);
                int lower=max(a[x-1][i].first,a[y-1][i].first);
                int temp=(upper-lower);
                if(temp>=0 && upper && lower) temp++;
                count+=max(0ll,temp);
            }
            cout<<count<<endl;
        }
