...
#define NO  cout<<"NO\n"
#define emo cout<<"('_')\n"
#define all(v) v.begin(),v.right()
#define rall(v) v.rbegin(),v.rright()
...
class SegmentTree{
    ...
    public:
        ...
        void push(int left,int righting,int node){
            if(lazyTree[node]){
        ...
        int query(int left,int righting,int l,int r,int node){
            push(left,righting,node);
            if (left>r || righting<l) return 0;
            if (left>=l && righting<=r) return segmentTree[node];
            int mid=(left+righting)/2;
            int q1=query(left,mid,l,r,2*node+1);
            int q2=query(mid+1,righting,l,r,2*node+2);
            return max(q1,q2);
        }
        void update(int left,int righting,int node,int l,int r,int value){
            push(left,righting,node);
            if (left>r || righting<l) return;
            if (left>=l && righting<=r){
                lazyTree[node]=value;
                push(left,righting,node);   
                return;
            }
            // partial case
            int mid=(left+righting)/2;
            update(left,mid,2*node+1,l,r,value);
            update(mid+1,righting,2*node+2,l,r,value);
...
int query(int l,int r,int index,int left,int right) {
    ...
    if (l<=left && right<=r) return st[index];
    int mid=(left+right)/2;
    int left_gcd=query(l,r,2*index+1,left,mid);
    int right_gcd=query(l,r,2*index+2,mid+1,right);
    return gcd(left_gcd,right_gcd);
}

int query(int l,int r) {
    return query(l,r,0,0,n-2);
}
