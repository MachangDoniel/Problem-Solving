...
int manhattanDistances(int n,int m,vector<vector<int>>& matrix) {
    map<int,vector<pair<int,int>>>colorPositions;
    for(int i=0;i<n;i++){
        for(int j=0;j<m;j++){
            colorPositions[matrix[i][j]].pb({i,j});
        }
    }
    int totalSum = 0;
    for(auto it : colorPositions) {
        auto positions = it.second;
        int k = positions.size();
        if (k < 2) continue; 
        vector<int> X(k), Y(k);
        for(int i=0;i<k;i++) {
            X[i] = positions[i].first;
            Y[i] = positions[i].second;
        }
        sort(all(X));
        int xSum = 0;
        for(int i=0;i<k;i++) {
...
void solve(int &t,int &T){
    ...
    int result=manhattanDistances(n,m,matrix);
    cout<<2*result<<endl;
