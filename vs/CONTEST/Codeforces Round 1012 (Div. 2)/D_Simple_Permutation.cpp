...
void solve(int &t,int &T){
    // ll in,n,m,i,j,k,x,y;
    int n; cin>>n;
    queue<int>primes=prime,rest;
    // print(primes);
    int prm=primes.front();
    primes.pop();
    int sum=0,sz=0;
    int count=0;
    for(int i=1;i<=n;i++){
        // cout<<prm<<" "<<(sum+i)/(sz+1)<<endl;
        if(prm==(sum+i)/(sz+1)){
            sum+=i;
            // cout<<prm<<" ";
            prm=primes.front();
