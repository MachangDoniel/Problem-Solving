...
// int nn=5*1e4+350;
set<pair<int,pair<int,int>>>new_tables,seats;
void pre(int nn){
    new_tables.clear();
    seats.clear();
    nn+=250;
    for(int i=1;;i++){
        ...
        int y=3*i-2;
        int d=y+1;
        new_tables.insert({d,{x,y}});
        for(int j=2;j<=i;j++){
            x+=3;
            y-=3;
            new_tables.insert({d,{x,y}});
            if(new_tables.size()>=nn) break;
        }
        if(new_tables.size()>=nn) break;
    }
    for(int i=1;;i++){
        ...
        int d=y+1;
        seats.insert({d,{x,y}});
        seats.insert({d+1,{x+1,y}});
        seats.insert({d+1,{x,y+1}});
        seats.insert({d+4,{x+1,y+1}});
        for(int j=2;j<=i;j++){
            x+=3;
            y-=3;
            seats.insert({d,{x,y}});
...
void solve(int &t,int &T){
    ...
    for(int i=0;i<n;i++){
        int in; cin>>in;
        if(in==0){
            int d=new_tables.begin()->first;
            int x=new_tables.begin()->second.first;
            int y=new_tables.begin()->second.second;
            new_tables.erase(new_tables.begin());
            auto it=seats.find({d,{x,y}});
            if(it!=seats.end()) seats.erase(it);
            cout<<x<<" "<<y<<endl;
        }
        else{
            int d=seats.begin()->first;
            int x=seats.begin()->second.first;
            int y=seats.begin()->second.second;
            seats.erase(seats.begin());
            auto it=new_tables.find({d,{x,y}});
            if(it!=new_tables.end()) new_tables.erase(it);
            cout<<x<<" "<<y<<endl;
        }
    }
}
