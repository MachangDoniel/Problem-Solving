...
void solve() {
    ...
    vector<int>v(n);
    map<int,int>parent;
    set<pair<int,pair<int,int>>>s;  // set<diff,<1st index,2nd index>>
    vector<int>mark(n,1);
    for(int i=0;i<n;i++){
        cin>>v[i];
        if(i){
    ...
    for(int i=0;i<n-k;i++){
        ...
        s.erase(it);
        int parent_x=min(parent[x],parent[y]);
        int parent_y=max(parent[x],parent[y]);
        parent[x]=parent_x;
        parent[y]=parent_y;
        mark[parent_x]+=mark[parent_y]+d-1;
        mark[parent_y]=0;
    }
    int sum=0;
    for(int i=0;i<n;i++){
