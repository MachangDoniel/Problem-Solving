...
void solve(int &t,int &T){
    ...
    for(int i=0;i<n;i++){
        points.pb(v[i]);
        if(i==n-1) points.pb(l);
    }
    dbg(points);
    vector<int>need;
    need.pb(points[0]);
    need.pb(points[n]-points[n-1]);
    for(int i=1;i<n;i++){
        need.pb((points[i]-points[i-1]+1)/2);
    }
    sort(all(need));
    dbg(need);
    vector<int>light(m);
    ...
    reverse(all(need));
    // cout<<need[0]<<endl;
    int index=lb(all(light),need[0])-light.begin();
    // cout<<index<<" "<<light[index]<<endl;
    if(index==m) cout<<-1<<endl;
    else cout<<light[index]<<endl;
}
