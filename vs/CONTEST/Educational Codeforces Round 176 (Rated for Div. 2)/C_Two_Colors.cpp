...
void solve(int &t,int &T){
    ...
    sort(all(a));
    vector<int>rest(m),prefixRest(m,0);
    for(int i=m-1,idx=0;i>=0;i--,idx++){
        rest[idx]=(n-a[i]);
        if(idx) prefixRest[idx]=prefixRest[idx-1];
        prefixRest[idx]+=rest[idx];
    }
    // print(prefixRest);
    int sum=0;
    for(int i=0;i<m-1;i++){
        int idx=ub(all(rest),a[i])-rest.begin()-1;
        if(m-idx-1<=i) idx=m-i-2;
        // cout<<idx<<": ";
        int add=max(0ll,(idx+1)*a[i]-prefixRest[idx])+(idx+1);
        // cout<<add<<" ";
        sum+=add;
    }
    // cout<<endl;
