...
void solve(int &t,int &T){
    // ll in,n,m,i,j,k,x,y;
    int x1,y1,z1,x2,y2,z2,k; cin>>x1>>y1>>z1>>x2>>y2>>z2>>k;
    int changefaceX=abs(x1-x2)/k+(abs(x1-x2)%k!=0);
    int changefaceY=abs(y1-y2)/k+(abs(y1-y2)%k!=0);
    int changefaceZ=abs(z1-z2)/k+(abs(z1-z2)%k!=0);
    int ans=abs(x1-x2)+abs(y1-y2)+abs(z1-z2);
    if(changefaceX>=changefaceY && changefaceX>=changefaceZ){
        int stepX=abs(x1-x2);
        int stepYZ=abs(y1-y2)+abs(z1-z2);
        if(stepYZ<changefaceX){
            int extra=changefaceX-stepYZ-1;
            extra+=extra%2;
            ans+=extra;
        }
    }
    else if(changefaceY>=changefaceX && changefaceY>=changefaceZ){
        int stepY=abs(y1-y2);
        int stepXZ=abs(x1-x2)+abs(z1-z2);
        if(stepXZ<changefaceY){
            int extra=changefaceY-stepXZ-1;
            extra+=extra%2;
            ans+=extra;
        }
    }
        ...
        int stepZ=abs(z1-z2);
        int stepXY=abs(x1-x2)+abs(y1-y2);
        if(stepXY<changefaceZ){
            int extra=changefaceZ-stepXY-1;
            extra+=extra%2;
            ans+=extra;
        }
    }
