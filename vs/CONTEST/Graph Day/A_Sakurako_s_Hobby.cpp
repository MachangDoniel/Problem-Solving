...
int traverse(int index,vector<int>&p,vector<int>&ans,vector<bool>&vis,string &s){
    // cout<<index<<" "<<p[index]<<endl;
    if(vis[index]){
        return ans[index]+0;
    }
    vis[index]=true;
    parents[p[index]]=parents[index];
    return ans[index]=traverse(p[index],p,ans,vis,s)+(s[index-1]=='0');
}
...
void solve(int &t,int &T){
    ...
    dbg(s);
    for(int i=1;i<=n;i++){
        if(!vis[i]) parents[i]=i;
        traverse(i,p,ans,vis,s);
        dbg(p);
        dbg(ans);
        dbg(vis);
        dbg(s);
    ...
    // dbg(parents);
    for(int i=1;i<=n;i++) cout<<ans[parents[i]]<<" ";
    cout<<endl;
}
