...
bool validHash(int i,int j){
    return i>=0 && i<n && j>=0 && j<m && !vis[i][j] && Graph[i][j]=='#';
}
bool valid(int i,int j){
    return i>=0 && i<n && j>=0 && j<m;
}
int set_size=0,parent=0;
void dfs(int i,int j){
    vis[i][j]=true;
    Set[i][j]=parent;
    set_size++;
    if(validHash(i+1,j)) dfs(i+1,j);
    if(validHash(i-1,j)) dfs(i-1,j);
    if(validHash(i,j+1)) dfs(i,j+1);
    if(validHash(i,j-1)) dfs(i,j-1);
}
...
void solve(int &t,int &T){
    ...
    for(int i=0;i<n;i++){
        ...
        for(int j=0;j<m;j++){
            ...
            else st.insert(Set[i][j]);
            if(valid(i+1,j) && Set[i+1][j]!=-1) st.insert(Set[i+1][j]);
            if(valid(i-1,j) && Set[i-1][j]!=-1) st.insert(Set[i-1][j]);
        }
        for(auto it:st){
            cnt+=parents[it];
    ...
    for(int i=0;i<m;i++){
        ...
        for(int j=0;j<n;j++){
            ...
            else st.insert(Set[j][i]);
            if(valid(j,i+1) && Set[j][i+1]!=-1) st.insert(Set[j][i+1]);
            if(valid(j,i-1) && Set[j][i-1]!=-1) st.insert(Set[j][i-1]);
        }
        for(auto it:st){
            cnt+=parents[it];
