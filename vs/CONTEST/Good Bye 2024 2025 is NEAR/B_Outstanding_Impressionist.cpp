...
void solve(int &t, int &T) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (l == r) {
            if (mp[l] == 1) 
                cout << 1; // Unique single occurrence.
            else 
                cout << 0; // Multiple occurrences, fail.
        } else {
            // Find the smallest value >= l in `has`
            auto it = lower_bound(all(has), l);
            if (it != has.end() && *it <= r) 
                cout << 1; // Found within range.
            else 
                cout << 0; // Not found within range.
        }
    }
    cout << endl;
}
