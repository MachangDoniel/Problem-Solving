...
void preprocess() {
    reachable.assign(m, vector<int>(m, 0));
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < m; j++) {
            if ((i - j) * (i - j) <= d * d) {
                reachable[i][j] = 1;
...
void solve() {
    ...
    for (int i = n - 2; i >= 0; i--) {
        ...
        for (auto &[prev_holds, cnt] : dp) {
            for (int mask = 1; mask < (1 << holds[i].size()); mask++) {
                vector<int> curr_holds;
                for (int k = 0; k < holds[i].size(); k++) {
                    if (mask & (1 << k)) {
                        curr_holds.push_back(holds[i][k]);
                    }
                }
                if (curr_holds.size() > 2) continue;
                
                bool valid = false;
                for (int ph : prev_holds) {
                    for (int ch : curr_holds) {
                        if (reachable[ph][ch]) {
                            valid = true;
                        }
                    }
                ...
                if (valid) {
                    new_dp[curr_holds] = (new_dp[curr_holds] + cnt) % MOD;
                }
            }
        }
        dp = move(new_dp);
