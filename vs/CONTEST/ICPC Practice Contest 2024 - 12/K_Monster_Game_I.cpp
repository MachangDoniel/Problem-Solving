...
bool good(int mid,int index){
    int prev=mid-1,sum1;
    int sum;
    int next=mid+1,sum2;
    if(valid(prev)) sum1=f[index]*ans[prev];
    else sum1=inf;
    if(valid(mid)) sum=f[index]*ans[mid];
    else sum=inf;
    if(valid(next)) sum2=f[index]*ans[next];
    else sum2=inf;
    if(sum1<=sum && sum<=sum2) return true; // increasing
...
void solve(int &t,int &T){
    // ll in,n,m,i,j,k,x,y;
    int ff; cin>>n>>ff;
    s.resize(n+1,ff),f.resize(n+1,0);
    ...
    for(int i=n-2;i>=0;i--){
        ...
        while(high>low+1){
            int mid=(high+low)/2;
            if(good(mid,i)) high=mid; // increasing
            else low=mid;             // decreasing
        }
        ans[i]=min(s[i]*f[low]+ans[low],s[i]*f[high]+ans[high]);
    }
