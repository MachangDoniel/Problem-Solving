...
void solve(int &t,int &T){
    ...
    int ans=0;
    for(int d=1;d<=min((int)3,n-2);d++){
        for(int m=1;m<=min((int)2,n-1-d);m++){
            int y=n-d-m;
            int ind=(n+1)/2;
            cout<<"dmy: "<<d<<" "<<m<<" "<<y<<endl;
            int dd=0,mm=0,yy=0,temp=0,dd0=0,mm0=0,yy0=0; //dd0 means lastdigit0
            cout<<"ind1: "<<ind<<" "<<temp<<endl;
            // dd
            if(d==1){
                if(ind>=1) dd=9,ind-=1; // 1->9
                // never happen
            }
            else if(d==2){
                if(ind>=2) dd=9*9,ind-=2; //11-19, 21-29, ..... 91-99 (lets say, last digit not be 0)
                if(ind>2) dd0=9; // X0 (X->1-9) // last digit 0
                // never happen 
            }
            else if(d==3){
                // if(ind>3) dd=201,ind-=3; // 100->300
                // else 
                if(ind>=3) dd=180,ind-=3; // 101-109, 111-119, 112-119, ...... 291-299 (lets say, last digit not be 0)
                if(ind>3) dd0=21; // XY0 // last digit 0
                // never happen
            }
            
            temp=dd;
            temp%=mod;
            cout<<"ind2: "<<ind<<" "<<temp<<endl;
            // mm
            if(m==1){
                if(ind>=1) mm=9,ind-=1; // 1-9
                else if(ind==0) mm=1,ind-=0; // dependent on dd
                tem*=mm;
                temp%=mod;
                // never happen
            }
            else if(m==2){
                if(ind>=2) mm=15,ind-=2; // 11-19, 21-26  // (lets say, last digit not be 0)
                if(ind>2){
                    mm0=2; // X0 // last digit 0
                } 
                else if(ind==1) mm=2,ind-=1; //11, 22
                else if(ind==0) mm=1,ind-=0; // dependent on dd
                // never happen
            }
            cout<<"ind3: "<<ind<<" "<<temp<<endl;
            // yy
            yy=bigmod(9,ind,mod);
            temp*=yy;
            temp%=mod;
            cout<<dd<<" "<<mm<<" "<<yy<<" "<<temp<<endl;
            ans+=temp;
        }
    }
