...
void call(priority_queue<int>pq,int learning_cost,int translator_cost,int total_cost){
    // cout<<"pq: ";
    // print(pq);
    // cout<<"->"<<learning_cost<<" "<<translator_cost<<" "<<total_cost<<endl;
    if(pq.empty()){
        // cout<<"cost: "<<total_cost<<endl;
        ans=min(ans,total_cost);
        return;
    }
    int population=pq.top();
    pq.pop();
    call(pq,learning_cost,translator_cost,total_cost+learning_cost);
    int add_cost=(translator_cost+population)*((translator_cost+population-1))/2-(translator_cost*(translator_cost-1))/2;
    call(pq,learning_cost,translator_cost+population,total_cost+add_cost);
}


void solve(int &t,int &T){
    // ll in,n,m,i,j,k,x,y;
    int n,learning_cost; cin>>n>>learning_cost;
    map<char,int>mp;
    string s; cin>>s;
    for(int i=0;i<n;i++){
        mp[s[i]]++;
    ...
    // print(pq);
    int translator_cost=1,total_cost=0;
    ans=INT_MAX;
    call(pq,learning_cost,translator_cost,total_cost);
    cout<<ans<<endl;
}
