...
bool good(int b){
    int sum=sum_value(b);
    int sumlow=sum_value(b-1);
    int sumhigh=sum_value(b+1);
    if(sumhigh>=sum && sum>=sumlow) return 1;
    else return 0;
}
...
void solve(int &t,int &T){
    ...
    cout<<min(sum_value(high),sum_value(low))<<endl;
}
