...
bool good(int num,string s){
    int not_match1=0,not_match2=0,n=s.size();
    // cout<<num<<": "<<endl;
    for(int i=0;i<n;i++){
        // cout<<s[i%num]<<" "<<s[n-num+i%num]<<" "<<s[i]<<endl;
        // cout<<i%num<<" "<<n-1-num+i%num<<" "<<i<<endl;
        if(s[i%num]!=s[i]) not_match1++;
        if(s[n-num+i%num]!=s[i]) not_match2++;
    }
    return (not_match1<=1 || not_match2<=1);
}
