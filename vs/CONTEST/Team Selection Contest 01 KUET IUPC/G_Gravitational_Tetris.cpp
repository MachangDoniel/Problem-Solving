...
char call(vector<int>&v){
    // cout<<"v: "; print(v);
    if(v[0]==4) return 'A';
    if(v[0]==1 && v[1]==3) return 'E';
    if(v[0]==3 && v[1]==1) return 'G';
    if(v[0]==2 && v[1]==2) return 'C';
    if(v[0]==1 && v[1]==2 && v[2]==1) return 'D';
    if(v[0]==2 && v[1]==1 && v[2]==1) return 'F';
    if(v[0]==1 && v[1]==1 && v[2]==2) return 'H';
    if(v[0]==1 && v[1]==1 && v[2]==1 && v[3]==1) return 'B';
    // return 'X';
}

void printer(vector<int>&v,vector<pair<char,int>>&moves){
    vector<int>ex(10);
    for(int i=0;i<moves.size();i++){
        int index=moves[i].second;
        if(moves[i].first=='A') ex[index-1]+=4;
        if(moves[i].first=='B') ex[index-2]+=1,ex[index-1]+=3;
        if(moves[i].first=='C') ex[index-2]+=2,ex[index-1]+=2;
        if(moves[i].first=='D') ex[index-3]+=1,ex[index-2]+=1,ex[index-1]+=2;
        if(moves[i].first=='E') ex[index-2]+=1,ex[index-1]+=3;
        if(moves[i].first=='F') ex[index-3]+=2,ex[index-2]+=1,ex[index-1]+=1;
        if(moves[i].first=='G') ex[index-2]+=3,ex[index-1]+=1;
        if(moves[i].first=='H') ex[index-3]+=1,ex[index-2]+=1,ex[index-1]+=2;
...
void solve(int &t,int &T){
    ...
    vector<int>v(10);
    int mx=0;
    for(int i=0;i<10;i++) cin>>v[i],mx=max(v[i],mx);
    vector<int>need(10);
    for(int target=mx+1;target<=mx+10;target++){
        int sum=0;
        for(int i=0;i<10;i++){
            need[i]=target-v[i];
            sum+=need[i];
        ...
        if(sum%4==0){
            ...
            for(int i=0;i<10;i++){
                pre+=need[i];
                int curr=need[i];
                if(pre<4) temp.pb(need[i]);
                else{
                    while(pre>=4){
                        if(pre==4){
                            temp.pb(curr);
                            char move=call(temp);
                            moves.pb({move,i+2-temp.size()});
                            temp.clear();
                            pre=0;
                        }
                        else{
