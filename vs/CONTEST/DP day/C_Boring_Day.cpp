...
int good(int diff,int l,int r){
    if(diff>=l && diff<=r) return 0;    // between l and r
    else if(diff<l) return -1;          // less than l
    else return 1;                      // greater than r
}
...
void solve(int &t,int &T){
    ...
    for(int left=0,right=1;left<=n && right<=n;){
        while(left<right && good(v[right]-v[left],l,r)==1){
            left++;
        }
        if(good(v[right]-v[left],l,r)==0){
            count++;
            // cout<<left<<" "<<right<<endl;
            left=right;
            right++;
        }
        if(good(v[right]-v[left],l,r)==-1){
            right++;
        }
    }
    cout<<count<<endl;
