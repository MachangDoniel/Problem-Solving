...
int summation(vector<int>&vv,int index1,int index2){
    return vv[index2]-vv[index1];
}

void solve(int &t,int &T){
    // ll in,n,m,i,j,k,x,y;
    int n,m; cin>>n>>m;
    vector<pair<int,int>>skills(n+m+1+1);
    vector<int>pp,pt,tp,tt;
    for(int i=1;i<=n+m+1;i++){
        cin>>skills[i].first;
    }
    pp.pb(0),pt.pb(0),tp.pb(0),tt.pb(0);
    vector<int>index(n+m+1+1,0);
    for(int i=1;i<=n+m+1;i++){
        cin>>skills[i].second;
        if(skills[i].first>skills[i].second){
            index[i]=pp.size();
            pp.pb(skills[i].first);
            pt.pb(skills[i].second);
        }
        else{
            index[i]=tt.size();
            tp.pb(skills[i].first);
            tt.pb(skills[i].second);
        }
    }
    ...
    dbg(tp);
    dbg(tt);
    int no_of_prog_candidates=pp.size()-1;
    int no_of_test_candidates=tt.size()-1;
    for(int i=1;i<=n+m+1;i++){
        int prog=skills[i].first, test=skills[i].second;
        bool programmer=prog>test;
        int sum=0;
        if(programmer){
            dbg("programmer: ");
            // i person who is a programmer ignored
            if(no_of_prog_candidates-1==n){
                dbg("n prog candidate, m tester candidate");
                // n prog candidate, m tester candidate
                // n programmer
                sum+=summation(pp,0,index[i]-1);
                sum+=summation(pp,index[i],pp.size()-1);
                // m tester
                sum+=summation(tt,0,tt.size()-1);
            }
            else if(no_of_prog_candidates-1<n){
                dbg("<n prog candidate, >m tester candidate");
                // n programmer
                sum+=summation(pp,0,index[i]-1);
                sum+=summation(pp,index[i],pp.size()-1);
                sum+=summation(tp,m,tp.size()-1);
                // m tester
                sum+=summation(tt,0,m);
            }
            else{
                dbg(">n prog candidate, <m tester candidate");
                // >n prog candidate, <m tester candidate
                if(index[i]-1>n){
                    // n programmer
                    sum+=summation(pp,0,n);
                    // m tester
                    sum+=summation(tt,0,tt.size()-1);
                    sum+=summation(pt,n,index[i]-1);
                    sum+=summation(pt,index[i],pt.size()-1);
                }
                else{
                    // n programmer
                    sum+=summation(pp,0,index[i]-1);
                    sum+=summation(pp,index[i],n+1);
                    // m tester
                    sum+=summation(tt,0,tt.size()-1);
                    sum+=summation(pt,n+1,pt.size()-1);
                }
            }
        }
        else{
            // i person who is a tester ignored
            dbg("tester: ");
            if(no_of_test_candidates-1==m){
                dbg("n prog candidate, m tester candidate");
                // n prog candidate, m tester candidate
                // n programmer
                sum+=summation(pp,0,pp.size()-1);
                // m tester
                sum+=summation(tt,0,index[i]-1);
                sum+=summation(tt,index[i],tt.size()-1);
            }
            else if(no_of_test_candidates-1<m){
                dbg(">n prog candidate, <m tester candidate");
                // >n prog candidate, <m tester candidate
                int need=m-(no_of_test_candidates-1);
                // n programmer
                sum+=summation(pp,0,n);
                // m tester
                sum+=summation(tt,0,index[i]-1);
                sum+=summation(tt,index[i],tt.size()-1);
                sum+=summation(pt,n,pt.size()-1);
            }
            else{
                dbg("<n prog candidate, >m tester candidate");
                // <n prog candidate, >m tester candidate
                int need=(no_of_test_candidates-1)-m;
                if(index[i]-1>m){
                    // m tester
                    sum+=summation(tt,0,m);
                    // n programmer
                    sum+=summation(pp,0,pp.size()-1);
                    sum+=summation(tp,m,index[i]-1);
                    sum+=summation(tp,index[i],tp.size()-1);
                }
                else{
                    // m tester
                    sum+=summation(tt,0,index[i]-1);
                    sum+=summation(tt,index[i],m+1);
                    // n programmer
                    sum+=summation(pp,0,pp.size()-1);
                    sum+=summation(tp,m+1,tp.size()-1);
                }
            }
