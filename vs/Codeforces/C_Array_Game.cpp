        ...
        if(k==1){
            ...
            for(int i=0;i<n-1;i++){
                for(int j=i+1;j<n;j++){
                    ...
                    ll high=n-1,low=0,index=-1;
                    while(high>=low){
                        ll mid=low+(high-low)/2;
                        if(v[mid]>=diff) high=mid-1,index=mid;
                        else low=mid+1;
                    }
                    if(index==-1) ans=min(ans,abs(diff-v[n-1]));
                    else ans=min(ans,abs(v[index]-diff));
                    if(index==0) ans=min(ans,abs(v[0]-diff));
                    else ans=min(ans,abs(diff-v[index-1]));
                }
            }
        }
        cout<<ans<<endl;
