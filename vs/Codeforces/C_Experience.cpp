...
ll parent[N],experience[N],Extra[N],Size[N];
...
void Union(ll n,ll m){
    ...
    if(n!=m){
        ...
        parent[m]=n;
        Size[n]+=Size[m];
        Extra[m]=experience[n];
    }
}
void update_ex(ll n,ll m){
    n=find(n);
    experience[n]+=m;
}
ll get(ll n){
    if(n==parent[n]) return experience[n];
    return experience[n]-Extra[n]+get(parent[n]);
}
...
int main()
{
    ...
    while(T--){
        ...
        for(ll i=0;i<m;i++){
            string s; cin>>s;
            if(s=="add"){
                ll node,amount; cin>>node>>amount;
                update_ex(node,amount);
            }
            else if(s=="join"){
