...
void solve(int &t,int &T){
    ...
    int n,a;
    cin>>n>>a;
    vector<vector<int>>v(100,vector<int>(n,0));
    auto legal=[&](int i,int j){
        return i>=0 && i<n && j>=0 && j<n && v[i][j]==0;
    };
    int num=1;
    if(a==1){
        for(int i=0;i<n;i++){
        ...
        v[x][y]=num++;
        while(1){
            if((!legal(x,y+1) && !legal(x+1,y) && !legal(x-1,y) && !legal(x,y-1))) break;
            while(legal(x,y+1)){
                y++;
                v[x][y]=num++;
            }
