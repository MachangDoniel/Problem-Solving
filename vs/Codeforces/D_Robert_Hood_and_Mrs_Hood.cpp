...
void solve(int &t,int &T){
    ...
    // }
    set<int>ms;
    int mx,mx_start,mn,mn_start;
    for(int i=1;i<=d;i++){
        for(auto it:in[i]) ms.insert(it);
    }
    mx=ms.size();
    mx_start=1;
    mn=ms.size();
    mn_start=1;
    // cout<<"ms:"; print(ms);
    // cout<<"mx:"<<mx<<" mx_start:"<<mx_start<<" mn:"<<mn<<" mn_start:"<<mn_start<<endl;
    for(int i=d+1;i<=n;i++){
        for(auto it:in[i]) ms.insert(it);
        for(auto it:out[i-d+1]) ms.erase(it);
        if(ms.size()>mx){
            mx=ms.size();
            mx_start=i-d+1;
        }
