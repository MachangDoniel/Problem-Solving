...
void solve(int &t,int &T){
    // ll in,n,m,i,j,k,x,y;
    int n,m,v; cin>>n>>m>>v;
    vector<int>a(n),pre(m+1,inf),suf(m+1,inf);
    int sum=0;
    for(int i=0;i<n;i++){
        cin>>a[i];
        sum+=a[i];
    }
    int index=0,blocksum=0;
    pre[index++]=0;
    for(int i=0;i<n;i++){
        if(index<=m){
            blocksum+=a[i];
            if(blocksum>=v){
                pre[index]=blocksum+pre[index-1];
                index++;
                blocksum=0;
            }
        }
    ...
    for(int i=n-1;i>=0;i--){
        if(index<=m){
            blocksum+=a[i];
            if(blocksum>=v){
                suf[index]=blocksum+suf[index-1];
                index++;
                blocksum=0;
            }
        }
    ...
    // dbg(pre);
    // dbg(suf);
    if(pre[m]==inf || suf[m]==inf){
        cout<<-1<<endl;
    }
    else{
        int mx=-1;
        for(int i=0;i<=m;i++){
            mx=max(mx,sum-pre[i]-suf[m-i]);
        }
        cout<<mx<<endl;
    }
}
