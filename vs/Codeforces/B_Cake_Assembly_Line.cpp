...
int main()
{
    ...
    while(T--){
        ...
        int n,w,h; cin>>n>>w>>h;
        vector<int>a(n),b(n);
        int min_move_first=INT_MAX, max_move_second=INT_MIN;
        for(int i=0;i<n;i++) cin>>a[i];
        for(int i=0;i<n;i++){
            cin>>b[i];
            min_move_first=min((b[i]-h)-(a[i]-w),min_move_first);
            max_move_second=max((b[i]+h)-(a[i]+w),max_move_second);
        }
        (min_move_first>=max_move_second)?YES:NO;
