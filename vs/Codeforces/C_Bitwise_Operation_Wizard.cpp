...
main()
{
    ...
    for(int t=1;t<=T;t++){
        // ll in,n,m,i,j,k,x,y;
        int n; cin>>n;
        // find maximum values Index
        int max_valueIndex=0;
        for(int i=1;i<n;i++){
            char s=ask(max_valueIndex,max_valueIndex,i,i);
            if(s=='<') max_valueIndex=i;
        }
        // find minimum values index which results max Xor with max_valueIndex
        int min_valueIndex_maxXorWith_max_valueIndex=0;
        for(int i=0;i<n;i++){
            char s=ask(max_valueIndex,min_valueIndex_maxXorWith_max_valueIndex,max_valueIndex,i);
            if(s=='<') min_valueIndex_maxXorWith_max_valueIndex=i;
            else if(s=='='){
                char s=ask(min_valueIndex_maxXorWith_max_valueIndex,min_valueIndex_maxXorWith_max_valueIndex,i,i);
                if(s=='>') min_valueIndex_maxXorWith_max_valueIndex=i; 
            }
        }
        cout<<"! "<<max_valueIndex<<" "<<min_valueIndex_maxXorWith_max_valueIndex<<endl;
        cout.flush();
