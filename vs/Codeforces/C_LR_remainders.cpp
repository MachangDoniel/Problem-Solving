...
main()
{
    ...
    for(int t=1;t<=T;t++){
        ...
        int mul=a[lastIndex]%m,rightIndex=lastIndex,leftIndex=lastIndex;
        mul%=m;
        ans[n-1]=mul;
        // cout<<lastIndex<<": "<<mul<<endl;
        for(int i=n-2;i>=0;i--){
            // cout<<lastIndex<<": "<<mul<<endl;
            if(s[i]=='R') leftIndex++,lastIndex=leftIndex;
            else rightIndex--,lastIndex=rightIndex;
            mul*=a[lastIndex]%m;
            mul%=m;
            ans[i]=mul;
