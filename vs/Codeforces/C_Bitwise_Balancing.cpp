...
void solve(int &t,int &T){
    ...
    int a=0;
    for(int i=0;i<64;i++){
        int bb=1ll&(b>>i);  // ith bit of b
        int cc=1ll&(c>>i);  // ith bit of c
        int dd=1ll&(d>>i);  // ith bit of d
        // cout<<bb<<" "<<cc<<" "<<dd<<" ";
        if(good(0,bb,cc,dd)){
            // cout<<"(0)"<<endl;
            a|=(0ll<<i);    // reset ith bit of a
        }
        else if(good(1,bb,cc,dd)){
            // cout<<"(1)"<<endl;
            a|=(1ll<<i);    // set ith bit of a
        }
        else{
            cout<<-1<<endl;
            return;
