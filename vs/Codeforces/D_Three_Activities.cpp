...
void check(int row,int first,int second,int third){
    if(row==3){
        ans=max(ans,v[0][first].first+v[1][second].first+v[2][third].first);
        // cout<<v[0][first].first<<" "<<v[1][second].first<<" "<<v[2][third].first<<endl;
        // cout<<v[0][first].second<<" "<<v[1][second].second<<" "<<v[2][third].second<<endl;
        // cout<<endl;
        return;
    }
    if(row==0){
        for(int i=0;i<3;i++){
            check(row+1,vv[row][i].second,second,third);
        }
    }
    if(row==1){
        for(int i=0;i<3;i++){
            if(vv[row][i].second!=first){
                check(row+1,first,vv[row][i].second,third);
            }
        }
    }
    if(row==2){
        for(int i=0;i<3;i++){
            if(vv[row][i].second!=first && vv[row][i].second!=second){
                check(row+1,first,second,vv[row][i].second);
            }
        }
    }
}
...
int main()
{
    ...
    while(T--){
        ...
        for(int i=0;i<3;i++){
            ...
            sort(all(c));
            for(int j=1;j<=3;j++){
                vv[i].pb({c[n-j].first,c[n-j].second});
            }
        }
        // for(int i=0;i<3;i++){
        //     for(int j=0;j<3;j++){
