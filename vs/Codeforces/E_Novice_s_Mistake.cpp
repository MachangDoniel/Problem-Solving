...
int eqn(int n,int mul,int num){
    return n*mul-(mul-num);
}
...
void solve(){
    ...
    while(n_str.size()<10000){
        n_str=n_str+n_str;
    }
    n_str=n_str.substr(0,10000);                                         
    vector<pair<int,int>>v;
    while(n_str.size()){
        // cout<<n_str<<" size: "<<n_str.size()<<endl;
        // cout<<n+1<<" "<<n+2<<endl;
        int first=eqn(n,n+1,n);
        int second=eqn(n,n+2,n);
        // int third=eqn(n,n+3,n);
        int diff=second-first;
        int target=atoi(n_str.c_str());
        // cout<<n_str<<"->"<<target<<" "<<first<<" "<<second<<" "<<third<<endl;
        int mul=(target-first);
        if(mul%diff==0){
            cout<<target<<endl;
            int a=1+mul/diff+n_str.size();
            int b=1+mul/diff;
            // cout<<a<<":: "<<b<<endl;
            if(0<a && a<=10000 && 0<b && b<=min(static_cast<long long>(10000),a*n)){
                v.pb({a,b});
            }
        }
        n_str.erase(n_str.begin()+n_str.size()-1);
    }
    sort(all(v));
    cout<<v.size()<<endl;
