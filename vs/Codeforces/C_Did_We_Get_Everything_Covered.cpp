...
int main()
{
    ...
    for(int t=1;t<=T;t++){
        ...
        for(int i=0;i<m;i++){
            ...
            int temp=(!found[s[i]-'a']);    // temp=1 if unique char in block, temp=0 if same char in block
            count+=temp;    // counting the unique char in block
            found[s[i]-'a']=true;   // true if exist in block
            if(count==k){
                mem(found,false);
                count=0;
                res+=s[i];  // saving the last elements of each block
            }
        }
        if(res.size()==n) YES;
        else{
            ...
            for(int i=0;i<k;i++){
                if(!found[i]){
                    while(res.size()<n){
                        res+=(char)('a'+i); // generating the char that are missing in unfinished blocks
                    }
                }
            }
            cout<<res<<endl;
