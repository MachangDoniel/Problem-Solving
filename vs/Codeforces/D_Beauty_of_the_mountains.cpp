...
void solve(){
    // ll in,n,m,i,j,k,x,y;
    int n,m,k; cin>>n>>m>>k;
    vector<vector<int>>v(n,vector<int>(m)),preHor(n+1,vector<int>(m+1,0)),preVer(n+1,vector<int>(m+1,0));
    for(int i=0;i<n;i++){
        for(int j=0;j<m;j++){
            cin>>v[i][j];
        }
    ...
    for(int i=0;i<n;i++){
        ...
        for(int j=0;j<m;j++){
            if(s[j]=='1'){
                sumOne+=v[i][j];
                preHor[i+1][j+1]=preHor[i+1][j]+1, preVer[i+1][j+1]=preVer[i][j+1]+1;
            }
            else{
                SumZero+=v[i][j];
                preHor[i+1][j+1]=preHor[i+1][j], preVer[i+1][j+1]=preVer[i][j+1];
            }
        }
    }
    // YES;
    int diff=SumZero-sumOne;
    set<int>s;
    for(int i=0;i<=n-k;i++){
        for(int j=0;j<=m-k;j++){
            int one=0,zero=0;
            for(int kk=i;kk<k+i;kk++){
                one+=preHor[kk+1][j+k]-preHor[kk+1][j];
            }
            s.insert(k*k-2*one);
        }
    }
