    ...
    for(int i=0;i<n;i++){
        for(int j=0;j<3;j++){
            ...
            if(j==0){
                ...
                // # # #
                if(i+3<=n){
                    int upVote=vote(s1[i],s1[i+1],s1[i+2]);
                    int downVote=vote(s2[i],s2[i+1],s2[i+2]);
                    dp[i+3][0]=max(dp[i+3][0],dp[i][0]+upVote+downVote);
                }
                // # 
                // # #
                if(i+2<=n){
                    int downRightVote=vote(s1[i],s2[i],s2[i+1]);
                    dp[i+2][1]=max(dp[i+2][1],dp[i][0]+downRightVote);
                }
                // # #
                // #
                if(i+2<=n){
                    int upRightVote=vote(s1[i],s1[i+1],s2[i]);
                    dp[i+2][2]=max(dp[i+2][2],dp[i][0]+upRightVote);
                }
            }
            // # # # # #
            //   # # # #
                ...
                //   # # #
                if(i+3<=n){
                    int upVote=vote(s1[i-1],s1[i],s1[i+1]);
                    int downVote=vote(s2[i],s2[i+1],s2[i+2]);
                    dp[i+3][1]=max(dp[i+3][1],dp[i][1]+upVote+downVote);
                }
                // # #
                //   #
                if(i+1<=n){
                    int rightDownVote=vote(s1[i-1],s1[i],s2[i]);
                    dp[i+1][0]=max(dp[i+1][0],dp[i][1]+rightDownVote);
                }
            }
            //   # # # #
            // # # # # #
                ...
                if(i+3<=n){
                    int upVote=vote(s1[i],s1[i+1],s1[i+2]);
                    int downVote=vote(s2[i-1],s2[i],s2[i+1]);
                    dp[i+3][2]=max(dp[i+3][2],dp[i][2]+upVote+downVote);
                }
                //   #
                // # #
                if(i+1<=n){
                    int rightUpVote=vote(s1[i],s2[i-1],s2[i]);
                    dp[i+1][0]=max(dp[i+1][0],dp[i][2]+rightUpVote);
                }
            }
        }
    } 
