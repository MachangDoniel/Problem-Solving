...
int check(vector<int>&a,int start,int end,int k){
    int sum=0;
    int length=0;
...
void solve(){
    ...
    // print(a);
    // print(h);
    int prevHeight=0,prevIndex=0,ans=0;
    for(int i=0;i<n;i++){
        // cout<<prevHeight<<" "<<h[i]<<endl;
        if(prevHeight%h[i]==0){
            // start from here.
        }
        else{
            // start from here.
            // at first, check the last subarray
            int length=check(a,prevIndex,i-1,k);
            // cout<<prevIndex<<" "<<i<<" "<<length<<endl;
            ans=max(ans,length);
            prevIndex=i;
        }
        if(i==n-1){
            int length=check(a,prevIndex,i,k);
            // cout<<prevIndex<<" "<<i<<" "<<length<<endl;
            ans=max(ans,length);
        }
        prevHeight=h[i];
