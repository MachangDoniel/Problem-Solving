...
void solve(){
    ...
    if(diff<=0) cout<<"Draw"<<endl;
        ...
        // case ALice
        if(diff%2){
            int moveA=diff/2+1,moveB=diff/2;
            int aright=min(w,ya+moveA);
            int bright=min(w,yb+moveB);
            int aleft=max(1ll,ya-moveA);
            int bleft=max(1ll,yb-moveB);
            // cout<<aright<<" "<<bright<<" "<<aleft<<" "<<bleft<<endl;
            (aright>=bright && aleft<=bleft)?cout<<"Alice"<<endl:cout<<"Draw"<<endl;
        }
        // case Bob
        else{
            int moveA=diff/2,moveB=diff/2;
            int aright=min(w,ya+moveA);
            int bright=min(w,yb+moveB);
            int aleft=max(1ll,ya-moveA);
            int bleft=max(1ll,yb-moveB);
            // cout<<aright<<" "<<bright<<" "<<aleft<<" "<<bleft<<endl;
            (aright<=bright && aleft>=bleft)?cout<<"Bob"<<endl:cout<<"Draw"<<endl;
        }
    }
}
