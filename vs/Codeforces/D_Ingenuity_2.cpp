...
void solve(){
    ...
    int n; cin>>n;
    string s; cin>>s;
    map<char,int> mp;       // 'N','S','E','W'
    for(int i=0;i<n;i++){
        mp[s[i]]++;
    }
    int subSN=abs(mp['S']-mp['N']),subEW=abs(mp['E']-mp['W']);
    if(subSN%2==0 && subEW%2==0){
        mpl helicopter,rober;
        {
            bool heli=true;
            while(mp['S']){
                if(heli) helicopter['S']++;
                else rober['S']++;
                mp['S']--;
                heli=!heli;
            }
        ...
        }
        {
            ...
            heli=false;
            while(mp['W']){
                if(heli) helicopter['W']++;
                else rober['W']++;
                mp['W']--;
                heli=!heli;
            }
        }
        // cout<<helicopter.size()<<" "<<rober.size()<<endl;
        if(helicopter.size() && rober.size()){
            for(int i=0;i<n;i++){
                if(helicopter[s[i]]) cout<<'H', helicopter[s[i]]--;
                else if(rober[s[i]]) cout<<'R', rober[s[i]]--;
            }
            cout<<endl;
        }
        else NO;
