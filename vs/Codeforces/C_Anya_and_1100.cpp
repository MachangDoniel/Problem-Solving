...
void solve(int &t,int &T){
    ...
    while(q--){
        ...
        if(ch=='1'){
            // include
            // first
            if(valid(pos) && s[pos]=='1'){
                if(valid(pos+1) && s[pos+1]=='1'){
                    if(valid(pos+2) && s[pos+2]=='0'){
                        if(valid(pos+3) && s[pos+3]=='0'){
                            ms.insert(pos);
                        }
                    }
                }
            ...
            // delete
            // third
            if(ms.find(pos-2)!=ms.end()){
                ms.erase(pos-2);
            }
            // fourth
            if(ms.find(pos-3)!=ms.end()){
                ms.erase(pos-3);
            }
        }
        if(ch=='0'){
            // include
            // third
            if(valid(pos-2) && s[pos-2]=='1'){
                if(valid(pos-1) && s[pos-1]=='1'){
                    if(valid(pos) && s[pos]=='0'){
                        if(valid(pos+1) && s[pos+1]=='0'){
                            ms.insert(pos-2);
                        }
                    }
            ...
            // delete
            // first
            if(ms.find(pos)!=ms.end()){
                ms.erase(pos);
            }
            // second
            if(ms.find(pos-1)!=ms.end()){
                ms.erase(pos-1);
            }
        }
        // cout<<s<<endl;
