...
main()
{
    ...
    for(int t=1;t<=T;t++){
        ...
        int n,k; cin>>n>>k;
        int a[n];
        map<int,int>mp; // store the mod of elements of (0/1/2/3/4) 
        for(int i=0;i<n;i++){
            cin>>a[i];
            a[i]%=k;
            mp[a[i]]++;
        ...
        if(mp[0]) cout<<0<<endl;
            ...
            if(k==2){
                ...
                if(mp[1]) ans=min(ans,2);
                if(mp[2]) ans=min(ans,1);
            }
            else if(k==4){
                if(mp[1]){
                    ans=min(ans,3);
                    if(mp[1]>1) ans=min(ans,2);
                    if(mp[2]) ans=min(ans,1);
                }
                if(mp[2]){
                    ans=min(ans,2);
                    if(mp[2]>1) ans=min(ans,0);
                }
                if(mp[3]){
                    ans=min(ans,1);
                }
