...
int find(vll &v,int index){
    int count=0,extra=0;
    for(int i=0;i<index;i++){
        if(v[i]>v[index]) return 0;
        extra=1;
    }
...
main()
{
    ...
    for(int t=1;t<=T;t++){
        ...
        // no swap, swap with 1st, swap with first >
        
        vv=v;
        int noswap=find(vv,k-1);

        vv=v;
        swap(vv[0],vv[k-1]);
        int swapwith1=find(vv,0);
        ...
        swap(vv[k-1],vv[firstGreater]);
        int swapwithfirstGreater=find(vv,firstGreater);
        
        // cout<<noswap<<" "<<swapwith1<<" "<<swapwithfirstGreater<<" "<<firstGreater<<" "<<vv[firstGreater]<<endl;
        cout<<max(noswap,max(swapwith1,swapwithfirstGreater))<<endl;
        // YES;
    }
}