...
void solve(){
    ...
    int n; cin>>n;
    vll v(n);
    map<pll,int>mp1,mp2,mp3;
    vector<msl>vv(3*n);
    int count=0;
    for(int i=0;i<n;i++){
        cin>>v[i];
        if(i>1){
            if(!mp1[{v[i-1],v[i]}]) mp1[{v[i-1],v[i]}]=++count;
            vv[mp1[{v[i-1],v[i]}]].insert(v[i-2]);
            if(!mp2[{v[i-2],v[i]}]) mp2[{v[i-2],v[i]}]=++count;
            vv[mp2[{v[i-2],v[i]}]].insert(v[i-1]);
            if(!mp3[{v[i-2],v[i-1]}]) mp3[{v[i-2],v[i-1]}]=++count;
            vv[mp3[{v[i-2],v[i-1]}]].insert(v[i]);
        }
    }
    int ans=0;
    for(int i=0;i<3*n;i++){
