...
main()
{
    ...
    for(int t=1;t<=T;t++){
        // ll in,n,m,i,j,k,x,y;
        int n,m,k; cin>>n>>m>>k;
        char vertical[n][m-1],horizontal[n-1][m];
        //  set default R
        for(int i=0;i<n;i++){
            for(int j=0;j<m-1;j++){
                vertical[i][j]='R';
        ...
        int extra=(k-(n+m-2));
        if(extra<0 || extra%2) NO;
        else{
            YES;
            bool red=true;  //R B R B ..... vertically
            for(int i=0;i<m-1;i++){
                vertical[0][i]=red?'R':'B';
                red^=1;
            }
            for(int i=0;i<n-1;i++){ // R B R ..... horizontally
                horizontal[i][m-1]=red?'R':'B';
                red^=1;
            }
            red^=1;
            if(extra%4==0){ // if divided by 4
                // cout<<"Div by 4"<<endl;
                vertical[n-2][m-2]=vertical[n-1][m-2]=red?'B':'R';
                horizontal[n-2][m-2]=red?'R':'B';
            }
            else{   // if divided by 2
                // cout<<"Div by 2"<<endl;
                vertical[n-2][m-2]=vertical[n-1][m-2]=red?'R':'B';
                horizontal[n-2][m-1]=horizontal[n-2][m-2]=red?'B':'R';
