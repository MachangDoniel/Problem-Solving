...
void solve(){
    ...
    vll bodya(n+1,0),sasha(n+1,0);

    // bodya's turn
    int kk=k,prev=0,bmax=0,smax=0;
    while(kk){
        int current=a[b]+prev;
        bmax=max(bmax,prev+kk*a[b]);    // all stay
        if(bodya[b]!=0) break;
        bodya[b]=current;   // one stay
    ...
    kk=k,prev=0;
    while(kk){
        int current=a[s]+prev;
        smax=max(smax,prev+kk*a[s]);    // all stay
        if(sasha[s]!=0) break;
        sasha[s]=current;   // one stay
    ...
    // print(bodya);
    // print(sasha);
    string ans=(bmax>smax)?"Bodya":(((bmax<smax))?"Sasha":"Draw");
    cout<<ans<<endl;
}
