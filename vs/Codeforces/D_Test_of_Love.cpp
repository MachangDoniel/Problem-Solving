...
void solve(){
    ...
    s='L'+s+'L';
    // cout<<s<<endl;
    vector<int>land,croc;
    for(int i=0;i<s.size();i++){
        if(s[i]=='L') land.pb(i);
        else if(s[i]=='C') croc.pb(i);
    }
    // print(land);
    // print(croc);
    int swim=0;
    int count=0;        // number of crocodiles i passed
    for(int i=1;i<land.size();i++){
        if(land[i]-land[i-1]<=m){                                   // just jump
            continue;
        }
        else{                 
            int reach=min(land[i-1]+m,land[i]);                                      // can't jump directly
            int crocodile_pos=lower_bound(croc.begin(),croc.end(),reach)-croc.begin();
            // cout<<land[i-1]<<"->"<<land[i]<<endl;
            if(crocodile_pos<croc.size()){
                // cout<<"crocodile_pos: "<<croc[crocodile_pos]<<endl;
                if(reach<=croc[crocodile_pos] && croc[crocodile_pos]<=land[i]){     // crocodile is in the middle fo the 2 safe land
                    NO;
                    return;
                }
            } 
            swim+=land[i]-land[i-1]-m;
        }
    }
    // cout<<"swim: "<<swim<<" "<<k<<endl;
    (swim<=k)?YES:NO;
