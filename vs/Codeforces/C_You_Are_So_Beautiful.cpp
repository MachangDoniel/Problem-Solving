...
main()
{
    ...
    for(int t=1;t<=T;t++){
        ...
        int n; cin>>n;
        int a[n];   // array
        map<int,int>mp1,mp2; // first appearance or not(mp1), last appearance or not(mp2)
        vector<int>first,last;
        for(int i=0;i<n;i++){
            cin>>a[i];
            if(!mp1[a[i]]) first.pb(i);
            mp1[a[i]]++;
        }
        for(int i=n-1;i>=0;i--){
            // cin>>a[i];
            if(!mp2[a[i]]) last.pb(i);
            mp2[a[i]]++;
        }
        sort(all(last));
        int ans=0;
        for(int i=0;i<first.size();i++){
            int val=first[i];
            int index=lb(all(last),val)-last.begin();
            int count=first.size()-index;
            ans+=count;
        }
        cout<<ans<<endl;
    }
