...
void solve(int &t,int &T){
    ...
    for(int i=0;i<q;i++){
        int l,r; cin>>l>>r; // 5 8
        int block_no=(l-1)/n;   // 1
        int index=l%n;      // 2
        index==0?index=n-1:index--; // 1
        index+=block_no;    // 2
        index%=n;           // 2          
        int global_index=l,sum=0;   // 5 0
        if(n-index<=r-global_index+1){
            sum+=pre[n]-pre[index];
            global_index+=n-index;
        }
        else sum+=pre[index+r-global_index+1]-pre[index],global_index=r;
        int count=(r-global_index+1)/n;
        sum+=count*(pre[n]-pre[0]);
        global_index+=count*n;
        // cout<<"Global Index: "<<global_index<<endl;
        if(global_index<r){
            int block_no=global_index/n;
            int index=global_index%n;
            index==0?index=n-1:index--;
            index+=block_no;
            index%=n;
            // cout<<":"<<index+r-global_index+1<<endl;
            sum+=pre[index+r-global_index+1]-pre[index];
            // cout<<sum<<endl;
        }
        cout<<sum<<endl;
    }
