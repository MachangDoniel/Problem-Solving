...
char traverse(int root,char hand){
    // cout<<endl<<"traversing: "<<root<<"("<<hand<<"): ";
    used[root]=true;
    char newhand=(hand=='R')?'H':'R';
    vector<char>v;
    if(adj[root].size()==1){
        // cout<<hand<<endl;
        return newhand;
    ...
    for(int i=0;i<adj[root].size();i++){
        if(used[adj[root][i]]==false){
            v.pb(traverse(adj[root][i],newhand));
        }
    }
    // cout<<endl<<"traversed: "<<root<<"("<<hand<<"): ";
    for(int i=0;i<v.size();i++){
        // cout<<v[i]<<" ";
        if(hand==v[i]) return hand;
    }
    return newhand;
}

void start(int root){
    // cout<<endl<<"traversing: "<<root<<"(R): ";
    used[root]=true;
    vector<char> ans;
    for(int i=0;i<adj[root].size();i++){
        ans.pb(traverse(adj[root][i],'H'));
    }
    // cout<<endl<<"traversed: "<<root<<"(R): ";
