...
void solve(){
    // ll in,n,m,i,j,k,x,y;
    int n,x; cin>>n>>x;
    vll cost(n+1),happiness(n+1);
    int sumofhappiness=0;
    for(int i=1;i<=n;i++){
        cin>>cost[i]>>happiness[i];
        sumofhappiness+=happiness[i];
    }
    vector<vll>dp(n+1,vll(sumofhappiness+1,-1));
    dp[0][0]=0;
    int res=0;
    for(int i=1;i<=n;i++){
        for(int j=0;j+happiness[i]<=sumofhappiness;j++){
            if(dp[i-1][j]>=cost[i]){
                dp[i][j+happiness[i]]=max(dp[i][j+happiness[i]],dp[i-1][j]-cost[i]);
            }
        }
        for(int j=0;j<=sumofhappiness;j++){
