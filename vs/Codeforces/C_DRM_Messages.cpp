...
int calculate_rotation(const string &s) {
...
string rotate_string(string &s,int rotation_value) {
    string result = s;
    for (int i = 0; i < s.size(); ++i) {
        result[i] = ((s[i] - 'A' + rotation_value) % 26) + 'A';
    }
    return result;
}
...
void solve(int &t,int &T){
    ...
    int n=s.size();
    string first=s.substr(0,n/2), sec=s.substr(n/2);

    int r_first=calculate_rotation(first);
    int r_secod=calculate_rotation(sec);

    string rotated_first=rotate_string(first,r_first);
    string rotated_sec=rotate_string(sec, r_secod);

    string result=merge_strings(rotated_first,rotated_sec);
    cout<<result<<endl;
}
