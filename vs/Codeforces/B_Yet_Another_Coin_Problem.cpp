...
// int find(int n){
//     int count=INT_MAX;
//     for(int ones=0;ones<=2;ones++){  // there could be at most 2 ones
//         for(int threes=0;threes<=1;threes++){ // there could be at most 1 threes
//             for(int sixes=0;sixes<=4;sixes++){  // there could be at most 4 sixes
//                 for(int tens=0;tens<=2;tens++){ // there could be at most 2 tens
//                     int sum=1*ones+3*threes+6*sixes+10*tens;
//                     if(sum<=n && (n-sum)%15==0) count=min(count,ones+threes+sixes+tens+(n-sum)/15);
//                 }
//             }
//         }
//     }
...
main()
{
    ...
    for(int t=1;t<=T;t++){
        // ll in,n,m,i,j,k,x,y;
        int n; cin>>n;
        if(n<10) cout<<find(n)<<endl;   // no tens
        else if(n<20) cout<<min(find(n),find(n-10)+1)<<endl; // no tens or 1 tens
        else cout<<min(find(n),min(find(n-10)+1,find(n-2*10)+2))<<endl; // no tens or 1 tens or 2 tens
