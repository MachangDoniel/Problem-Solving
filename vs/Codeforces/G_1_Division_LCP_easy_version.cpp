...
// MyTask
bool good(int mid,string s,int l,DoubleHash &dh){
    int count=0,n=s.size();
    int val=dh.range_hash(0,mid-1);
    // cout<<val<<endl;
    for(int i=0;i<s.size();i++){
        if(i+mid-1<n && dh.range_hash(i,i+mid-1)==val){
            l--;
            i+=(mid-1);
        }
    }
...
void solve(int t,int T){
    ...
    string s; cin>>s;
    DoubleHash dh(s);
    int high=n/l+1,low=0;
    while(high>low+1){
        int mid=(high+low)/2;
        if(good(mid,s,l,dh)) low=mid;
        else high=mid;
