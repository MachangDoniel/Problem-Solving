...
void solve(){
    ...
    for(int i=0;i<n/2;i++){
        ...
        for(int j=i;j<n-i-1;j++){
            // cout<<i<<","<<j<<" "<<j<<","<<n-i-1<<" "<<n-j-1<<","<<i<<" "<<n-i-1<<","<<n-j-1<<endl;
            char target=max(max(s[i][j],s[j][n-i-1]),max(s[n-j-1][i],s[n-i-1][n-j-1]));
            int count=(target-s[i][j])+(target-s[j][n-i-1])+(target-s[n-j-1][i])+(target-s[n-i-1][n-j-1]);
            sum+=count;
        }
    }
    cout<<sum<<endl;
