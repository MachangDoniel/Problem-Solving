...
void modified_dijkstra(int source,int n,vector<vector<pair<int,int>>>&adj,vector<int>&s){
    // cout<<"s: "; print(s);
    // cout<<"adj: "<<endl;
    ...
    vector<vector<bool>>vis(n,vector<bool>(n,false)); // visited array vis[parent][bike_no]
    vector<vector<int>>dis(n,vector<int>(n,inf));     // distance array dist[parent][bike_no]
    set<pair<pair<int,int>,int>>st;                   // {distance,parent},bike_no    
    int destination=n-1;
    dis[source][0]=0;                           
    st.insert({{0,source},0});
    while(!st.empty()){
        int distance=st.begin()->first.first;
        int parent=st.begin()->first.second;
        int bike_no=st.begin()->second;
        st.erase(st.begin());
        if(vis[parent][bike_no]) continue;
        vis[parent][bike_no]=true;
        for(auto child_pair: adj[parent]){
            int child=child_pair.first;         // child
            int w=child_pair.second;            // weight or cost or slowness_factor
            if(s[bike_no]<s[child]){            // if current bike has low slowing_factor than child_nodes bike
                if(dis[parent][bike_no]+s[bike_no]*w<dis[child][bike_no]){
                    dis[child][bike_no]=dis[parent][bike_no]+s[bike_no]*w;
                    st.insert({{dis[child][bike_no],child},bike_no});
                }
            }
            else{
                if(dis[parent][bike_no]+s[bike_no]*w<dis[child][child]){
                    dis[child][child]=dis[parent][bike_no]+s[bike_no]*w;
                    st.insert({{dis[child][child],child},child});
                }
            }
        }
    }
    ...
    for(int i=0;i<n;i++){
        ...
        ans=min(ans,dis[destination][i]);     // calculate distance to go to destination with i'th bike
    }
    cout<<ans<<endl;
}
