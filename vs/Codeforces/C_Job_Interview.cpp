...
void solve(){
    ...
    // vector<pair<int,int>>prefix_sum_prog(n+m+2,{0,0}),prefix_sum_test(n+m+2,{0,0});
    ...
    vector<pair<int,int>>pre_prog(prog.size()+1,{0,0}),pre_test(test.size()+1,{0,0});
    for(int i=prog.size();i>0;i--){
        // cout<<prog[i-1].first<<" "<<prog[i-1].second<<endl;
        pre_prog[i-1].first=pre_prog[i].first+prog[i-1].first;
        pre_prog[i-1].second=pre_prog[i].second+prog[i-1].second;
    }
    for(int i=test.size();i>0;i--){
        // cout<<test[i-1].first<<" "<<test[i-1].second<<endl;
        pre_test[i-1].first=pre_test[i].first+test[i-1].first;
        pre_test[i-1].second=pre_test[i].second+test[i-1].second; 
    }
    // reverse(all(pre_prog));
    // reverse(all(pre_test));
    // cout<<"pre_prog"<<endl;
    // for(int i=0;i<pre_prog.size();i++){
    //     cout<<pre_prog[i].first<<","<<pre_prog[i].second<<" ";
    // }
    // cout<<endl;
    // cout<<"pre_test"<<endl;
    // for(int i=0;i<pre_test.size();i++){
    //     cout<<pre_test[i].first<<","<<pre_test[i].second<<" ";
    // }
    // cout<<endl;
    for(int i=0;i<n+m+1;i++){
        int tot_prog=prog.size();
        int tot_test=test.size();
        
        int sum1=pre_prog[0].first;
        int sum2=pre_test[0].second;
        // cout<<sum1<<": "<<sum2<<endl;
        if(a[i]>b[i]){              // hence i is programmer
            tot_prog--;
            sum1-=a[i];
        } 
        else{                       // hence i is tester
            tot_test--;
            sum2-=b[i];
        }
        // cout<<tot_prog<<" "<<tot_test<<" "<<n<<" "<<m<<" "<<sum1<<" "<<sum2<<endl;
        if(tot_prog>n){
            int minus=0;
            if(mp_prog[pre_prog.size()-(tot_prog-n)-1]<i) minus++;
            sum1-=pre_prog[pre_prog.size()-(tot_prog-n)-1+minus].first;
            sum2+=pre_prog[pre_prog.size()-(tot_prog-n)-1+minus].second;
        }
        else if(tot_prog<n){
            int minus=0;
            if(mp_test[pre_test.size()-(n-tot_prog)-1]<i) minus++;
            sum1+=pre_test[pre_test.size()-(n-tot_prog)-1+minus].first;
            sum2-=pre_test[pre_test.size()-(n-tot_prog)-1+minus].second;
        }
        cout<<sum1+sum2<<" ";
        // YES;
    }
