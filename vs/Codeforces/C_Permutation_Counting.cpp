...
void ans(int mn,int sz,int extra){
    cout<<(mn-1)*sz+1+extra<<endl;
}
...
void solve(int test){
    ...
    // print(v);
    int mn=v[v.size()-1];
    int cost=(n-mp[mn])*v[v.size()-1]-(sum-mp[mn]*mn);
    // if overflow, set to v[v.size()-1]
    if(k>=cost){
        k-=cost;
        // still overflow
        if(k>=n){
    ...
    // if between the arrays smaller than v[v.size()-1]
    else{
        vll prefix(v.size()+1,0);
        for(int i=0;i<v.size();i++){
            prefix[i+1]=prefix[i]+v[i]*mp[v[i]];
        }
        // print(prefix);
        int count=n;
        for(int i=v.size()-1;i>=0;i--){
            int need=v[i]*count-prefix[i+1];    // to make all elements equals to v[i] that are <=v[i]
            // cout<<k<<" "<<need<<" "<<v[i]<<endl;
            if(k>=need){
                mn=v[i];
                k-=need;
                // cout<<":"<<k<<": "<<count<<endl;
