...
int main()
{
    ...
    while(T--){
        ...
        for(int i=0;i<n;i++){
            int sum=getSum(v[i]);
            ans+=dp[v[i].size()][sum];
            int removed_sum=0,count=0,left,right,left_sum,right_sum,remaining_left,remaining_right;
            for(int j=v[i].size()-1;j>=0;j--){
                removed_sum+=(v[i][j]-'0');
                count++;
                left=v[i].size()-count;
                left_sum=sum-removed_sum;
                remaining_right=left-count;
                right_sum=left_sum-removed_sum;
                if(remaining_right>=0 && right_sum>=0 && (left+remaining_right+count)%2==0) ans+=dp[remaining_right][right_sum];
            }
            removed_sum=0,count=0;
            for(int j=0;j<v[i].size();j++){
                removed_sum+=(v[i][j]-'0');
                count++;
                right=v[i].size()-count;
                right_sum=sum-removed_sum;
                remaining_left=right-count;
                left_sum=right_sum-removed_sum;
                if(remaining_left>=0 && left_sum>=0 && (right+remaining_left+count)%2==0) ans+=dp[remaining_left][left_sum];
            }
        }
        cout<<ans<<endl;
