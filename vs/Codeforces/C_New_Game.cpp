...
int time_limit=1e9,time_mp=0;
bool checkTime(){
    time_mp++;
    if(time_mp>time_limit){
...
void solve(int &t,int &T){
    ...
    vector<pair<int,int>>uniqueCards(all(mp));
    sort(all(uniqueCards));
    int maxCards=0,count=0,total=0,left=0;
    for (int right=0;right<uniqueCards.size();right++) {
            if (right>0 && uniqueCards[right].first!=uniqueCards[right-1].first+1){
                left=right;
                total=0;
                count=0;
            }
            total+=uniqueCards[right].second;
            count++;
            while (count>k) {
                total-=uniqueCards[left].second;
                left++;
                count--;
            }
            maxCards=max(maxCards,total);
        }
        cout<<maxCards<<endl;
}
