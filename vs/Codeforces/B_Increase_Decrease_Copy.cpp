...
void solve(){
    ...
    for(int i=0;i<n;i++){
        if((a[i]<=b[n] && b[n]<=b[i]) || (a[i]>=b[n] && b[n]>=b[i])) flag=true;
        min_diff=min(min_diff,min(abs(b[n]-a[i]),abs(b[n]-b[i])));
        sum+=abs(a[i]-b[i]);
    }
    if(flag) cout<<sum+1<<endl;         // copy operation
    else cout<<sum+min_diff+1<<endl;
}
