...
bool good(int first,int l,int r,vll &pre,vll &v){
    // initial xor till first
    int xor1=pre[l+first]^pre[l-1];
    // xor from first+1 to last
    int xor2=pre[r]^pre[l-1];
    cout<<xor1<<" "<<xor2<<endl;
    cout<<l-1<<" "<<l+first<<" "<<r<<endl;
    if(xor1==xor2){
        for(int i=l+first+1;i<=r;i++){
...
void solve(int t,int T){
    ...
    for(int i=0;i<q;i++){
        ...
        // if k==even, xor(till last)=0
        int see=pre[r]^pre[l-1];
        if(see==0){
            // cout<<"even: ";
            YES;
            // cout<<pre[r]<<" "<<pre[l-1]<<" "<<see<<endl;
        }
        // if k==odd, find if xor(till between)=xor(till last) exist or not for even as odd=even+1
        else{
            bool flag=false;
            for(int add=0;add<r-l+1;add++){
                if(good(add,l,r,pre,v)){
                    flag=true;
                    break;
                }
            }
