...
// MyTask
// ask a if b is knight
bool ask(int a,int b){
    cout<<"? "<<a<<" "<<b<<endl;
    cout.flush();
    bool knight; cin>>knight;
    return knight;
}
void ans(int imposter){
    cout<<"! "<<imposter<<endl;
    cout.flush();
}
...
void solve(int &t,int &T){
    ...
    int n; cin>>n;
    int suspect=1;
    while(suspect<n && ask(suspect,suspect+1)==ask(suspect+1,suspect)){
        suspect+=2;
    }
    // while loop break if any of suspect or suspect+1 is "imposter" or or or suspect is last
    // if(suspect==n) ans(suspect);
    int random=1;
    while(suspect==random || suspect+1==random) random++;
    if(ask(random,suspect)!=ask(suspect,random)) ans(suspect);
    else ans(suspect+1);
}
