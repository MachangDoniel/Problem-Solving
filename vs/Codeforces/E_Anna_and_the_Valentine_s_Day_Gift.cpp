...
void solve(int &t,int &T){
    ...
    vector<int>zeroes;
    int total_length=0;
    auto zeroAndLength = [](int num) -> pair<int,int>{
        int zero = 0, cnt = 0;
        while (num % 10 == 0) zero++, num /= 10;
        while (num > 0) cnt++, num /= 10;
        return {zero, zero + cnt};
    };
    for(int i=0;i<n;i++){
        int x; cin>>x;
        pair<int,int>p=zeroAndLength(x);
        total_length+=p.second;
        if(p.first>0) zeroes.push_back(p.first);
    }
    sort(all(zeroes));
    dbg(zeroes);
    for(int i=zeroes.size()-1;i>=0;i-=2){
        total_length-=zeroes[i];
    }
    if(total_length-1>=m) cout<<"Sasha"<<endl;
    else cout<<"Anna"<<endl;
}
