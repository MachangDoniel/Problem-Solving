...
void solve(){
    ...
    vll v(n);
    vector<pll>odd,even;
    int posof1; // minimize the odd or even where 1 exists, so in min side, there is max sum is n+1
    for(int i=0;i<n;i++){
        cin>>v[i];
        if(v[i]==1) posof1=i%2;
        if(i%2) odd.pb({v[i],i});
    ...
    mpl res;
    if(posof1){
        // 1 is in odd position, so minimize it
        int num=0;
        reverse(all(odd));
        for(int i=0;i<odd.size();i++){
            res[odd[i].second]=++num;
        }
        reverse(all(even));
        for(int i=0;i<even.size();i++){
            res[even[i].second]=++num;
        }
    }
    else{
        // 1 is not in odd position, so maximize it
        int num=0;
        reverse(all(even));
        for(int i=0;i<even.size();i++){
            res[even[i].second]=++num;
