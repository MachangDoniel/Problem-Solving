...
main()
{
    ...
    for(int t=1;t<=T;t++){
        ...
        for(int i=1;i<=n;i++){
            ...
            vector<int>diff;
            diff.pb(v[i][0]-1); // 1st step
            for(int j=1;j<v[i].size();j++){ // intermediate step
                diff.pb(v[i][j]-v[i][j-1]-1);
            }
            diff.pb(n-v[i][v[i].size()-1]); //last step
            sort(all(diff));
            // printV(diff);
            int local_result=max(diff[diff.size()-1]/2,diff[diff.size()-2]);
            // cout<<"color: "<<i<<" "<<local_result<<endl;
