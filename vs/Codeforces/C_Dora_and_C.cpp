...
void solve(int &t,int &T){
    ...
    int first=min(a,b);
    int second=max(a,b);
    s.insert(first),s.insert(second);
    a=first,b=second;
    while(s.size()>1){
        auto it=s.begin();
        a=*it;
        ...
        b=*it;
        int d=b%a;
        if(d==0 or s.find(d)!=s.end()){
            break;
        }
        s.insert(d);
    }
    // cout<<t<<": ";print(s);
    auto go = [&](int element, int target) -> int {
        vector<pair<int,int>>vv;
        for(int it:s){
           if(it>target-element){
               break;
           }
           vv.push_back({(target-element)%it,it});
        }
        sort(all(vv));
        int new_element=((target-element)%vv[0].second)*vv[0].first+element;
        return new_element; 
    };
    int target=v[n-1];
    for(int i=0;i<n;i++){
