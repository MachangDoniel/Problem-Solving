...
void solve(){
    ...
    int onlyplus=0,onlyminus=0,common=0;
    for(int i=0;i<n;i++){
        onlyplus+=v[i]/2+1;
        onlyminus+=(c-v[i]+1);
        if(v[i]%2) common+=ub(all(odd),v[i])-lb(all(odd),0);
        else common+=ub(all(even),v[i])-lb(all(even),0);
    }
    cout<<total-onlyplus-onlyminus+common<<endl;
}
