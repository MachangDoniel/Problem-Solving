...
void solve(int &t,int &T){
    ...
    for(int i=1;i<=n;i++){
        cin>>v[i];
        if(i!=v[i]) mp[min(i,v[i])]=max(i,v[i]);
    }
    string s; cin>>s;
    map<int,int>mpneed,mphave;
    multiset<int>need;
    int last=0;
    for(auto it:mp){
        ...
        for(int i=start+1;i<=end;i++){
            mpneed[i-1]=1;
            if(need.find(i-1)==need.end()) need.insert(i-1);
        }
        last=max(last,end);
    }
    dbg(need);
    for(int i=0;i<n;i++){
        ...
        if(s[i]=='L'){
            mphave[index-1]++;
            if(need.find(index-1)!=need.end()){
                need.erase(need.find(index-1));
            }
        }
        else{
            mphave[index]++;
            if(need.find(index)!=need.end()){
                need.erase(need.find(index));
            }
        }
    }
    dbg(msneed);
    ...
    for(int i=0;i<q;i++){
        ...
        if(ch=='L'){
            ...
            mphave[index-1]--;
            if(mpneed[index-1] && need.find(index-1)==need.end() && mphave[index-1]==0){
                need.insert(index-1);
            }
        }
        else{
            ...
            mphave[index]--;
            if(mpneed[index] && need.find(index)==need.end() && mphave[index]==0){
                need.insert(index);
            }
        }
        
        (need.empty())?YES:NO;
        ch=='L'?s[index-1]='R':s[index-1]='L';
        // cout<<s<<endl;
        dbg(msneed);
        dbg(mphave);
        dbg(need);
