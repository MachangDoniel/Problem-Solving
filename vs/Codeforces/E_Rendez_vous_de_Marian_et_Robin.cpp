...
vector<pair<int,int>>costfrom1(N,{0,-1}),costformN(N,{0,-1});     // with horse, without horse

void dijkstra(int node,int initial_cost){
    priority_queue<pair<int,int>>pq;
    pq.push({-initial_cost,node});
    while(!pq.empty()){
        ...
        for(int i=0;i<Adj[parent].size();i++){
            ...
            int child_cost=-Adj[parent][i].first;
            if(costfrom1[child].first==-1 || costfrom1[child].first>cost+child_cost){
                costfrom1[child].first=cost+child_cost;
                pq.push({-costfrom1[child].first,child});
            }
        }
    }
}
