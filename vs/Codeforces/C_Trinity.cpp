...
// solution idea: n - largest sorted subarray that satisfy the triangle inequality
...
void solve(int &t,int &T){
    ...
    int ans=0;
    for(int i=1;i<n-1;i++){
        int first=v[i-1],second=v[i];
        int index=lb(all(v),first+second)-v.begin();
        index--;
        ans=max(ans,index+2-i);
    }
    cout<<n-ans<<endl;
}
