...
vector<int> findCost(){
    ...
    for(int i=0;i<n;i++){
        ...
        ms.insert(1);
        for(int j=1;j<m;j++){
            int remove_index=max(-1ll,j-d-2);
            if(remove_index>=0){
                ms.erase(ms.find(dp[remove_index]));
            }
            dp[j]=*ms.begin()+a[i][j]+1;
            ms.insert(dp[j]);
        }
        cost[i]=dp[m-1];
        // for(int i=0;i<m;i++){
...
int findMinCost(vector<int>cost,int k){
    
    for(int i=1;i<cost.size();i++){
        cost[i]+=cost[i-1];
    }
    int min_cost=cost[k-1];
    for(int i=k;i<cost.size();i++){
        // cout<<cost[i]<<" "<<cost[]
        min_cost=min(min_cost,cost[i]-cost[i-k]);
    }
    return min_cost;
}
...
main()
{
    ...
    for(int t=1;t<=T;t++){
        ...
        vector<int>cost=findCost();
        int min_cost=findMinCost(cost,k);
        // printV(cost);
        cout<<min_cost<<endl;
    }
}