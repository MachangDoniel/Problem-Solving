...
main()
{
    ...
    for(int t=1;t<=T;t++){
        ...
        int n; cin>>n;
        string s; cin>>s;
        int mn=0,one=0,rone=0,zero=0,rzero=0;
        for(int i=0;i<n;i++){
            if(s[i]=='0') rzero++;
            else rone++;
        }
        vector<int>v;
        if(rzero<=rone && zero>=one) v.pb(0);
        for(int i=0;i<n;i++){
            if(s[i]=='0') rzero--,zero++;
            else rone--,one++;
            if(rzero<=rone && zero>=one) v.pb(i+1);
        }
        // printV(v);
        float distance=n;
        int ans=0;
        for(int i=0;i<v.size();i++){
            float d=abs(float(n)/2-v[i]);
            if(d<distance) distance=d,ans=v[i];
        }
        cout<<ans<<endl;
    }
}