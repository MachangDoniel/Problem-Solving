...
bool goodmin(int n,int r){
    if(n*n+n*n>=r*r) return true;   // sqrt(2*n*n)>=r
    else return false;
}
bool good(int x,int y,int r){
    if((int)sqrt(x*x+y*y)<(r+1)) return true;
    else return false;
}
bool good2(int x,int y,int r){
    if((int)sqrt(x*x+y*y)>=r) return true;
    else return false;
}
...
void solve(){
    ...
    // find mn
    {
        ...
        while(high>low+1){
            int mid=(high+low)/2;
            if(goodmin(mid,r)) high=mid;
            else low=mid;
        }
        mn=high; // sqrt(2*n*n)>=r
    }
    // find mx
    {
        mx=r; // til r
    ...
    for(int i=mn;i<=mx;i++){
        // cout<<i<<" : ";
        int high=i+1,low=-1,minus=0;
        while(high>low+1){
            int mid=(high+low)/2;
            if(good(i,mid,r)) low=mid;
            else high=mid;
        }
        int high2=i+1,low2=-1;
        while(high2>low2+1){
            int mid=(high2+low2)/2;
            if(good2(i,mid,r)) high2=mid;
            else low2=mid;
        }
        // cout<<high2<<" "<<low<<endl;
        if(i==low) minus=4;
        int oneside=2*(low-high2)+1;
        if(high2!=0) oneside++;
        count+=oneside*4-minus;
    }
    cout<<count<<endl;
}
