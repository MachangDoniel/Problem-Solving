...
bool found(string x, string y){
    if(x[0]==y[0] || x[0]==y[1] || x[1]==y[0] || x[1]==y[1]) return true;
    return false;
}
bool findInMid(vector<int>v,int x,int y){
    // print(v);
    int up=upper_bound(all(v),x)-v.begin();
    // cout<<x<<" "<<up<<" "<<y<<endl;
    return up>=0 && up<v.size() && v[up]<y;
    
}
bool findInLast(vector<int>v,int x,int y){
    int up=upper_bound(all(v),x)-v.begin();
    return up>=0 && up<v.size() && v[up]>y;
}
bool findInFirst(vector<int>v,int x,int y){
    int up=lower_bound(all(v),x)-v.begin();
    up--;
    return up>=0 && up<v.size() && v[up]>=0;
}
...
void solve(int &t, int &T) {
    ...
    for (int i = 0; i < n; i++) {
        cin >> v[i];
        sort(all(v[i])); // Sort the characters in each string
        mpv[v[i]].pb(i); // Map sorted strings to their indices
    }
    ...
    while (q--) {
        ...
        if (found(v[x], v[y])) {
            // Direct match found, return the absolute difference
            cout << abs(x - y) << endl;
            continue;
        }

        char s1 = v[x][0], s2 = v[x][1], d1 = v[y][0], d2 = v[y][1];
        int ans = inf;

        auto check_combinations = [&](char a1, char a2, char b1, char b2) {
            string s = {a1, b1};
            sort(all(s));

            if (mpv[s].empty()) return;

            auto it = upper_bound(all(mpv[s]), x);

            // Case 1: Find in the middle of the range
            if (it != mpv[s].end() && *it < y) {
                ans = min(ans, abs(x - y));
            }

            // Case 2: Check for positions greater than y
            if (it != mpv[s].end() && *it > y) {
                ans = min(ans, abs(*it - x) + abs(*it - y));
            }

            // Case 3: Check for positions less than x
            if (it != mpv[s].begin()) {
                it--;
                if (*it < x) {
                    ans = min(ans, abs(*it - x) + abs(*it - y));
        ...
        // Checking all possible character combinations between v[x] and v[y]
        check_combinations(s1, s2, d1, d2);
        check_combinations(s1, s2, d2, d1);
        check_combinations(s2, s1, d1, d2);
        check_combinations(s2, s1, d2, d1);
