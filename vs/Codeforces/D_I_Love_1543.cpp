...
void solve(int &t, int &T) {
    ...
    string target = "1543"; // The sequence we're looking for
    int count = 0;

    for (int i = 0; i < min(n / 2, m / 2); i++) {
        string extracted; // String to store elements as we traverse each layer
        ...
        // Check for all occurrences of '1543' in a circular manner
        int len = extracted.length();
        for (int start = 0; start < len; start++) {
            bool match = true;
            for (int k = 0; k < 4; k++) {
                if (extracted[(start + k) % len] != target[k]) {
                    match = false;
                    break;
                }
            }
