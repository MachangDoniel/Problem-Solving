...
bool good(int n,int mid,int k){
    int presum=(mid+k-1)*(mid+k)/2-k*(k-1)/2;;
    int postsum=(n+k)*(n+k-1)/2-presum-k*(k-1)/2;
...
void solve(int &t,int &T){
    ...
    while(high>low+1){
        int mid=(high+low)/2;
        if(good(n,mid,k)) low=mid;
        else high=mid;
    }
    int presum1=(low+k-1)*(low+k)/2-k*(k-1)/2;;
    int postsum1=(n+k)*(n+k-1)/2-presum1-k*(k-1)/2;
    // cout<<low<<"-> "<<presum1<<" "<<postsum1<<endl;
    low++;
    int presum2=(low+k-1)*(low+k)/2-k*(k-1)/2;;
    int postsum2=(n+k)*(n+k-1)/2-presum2-k*(k-1)/2;
    // cout<<low<<"-> "<<presum2<<" "<<postsum2<<endl;
    cout<<min(abs(postsum1-presum1),abs(postsum2-presum2))<<endl;
}
