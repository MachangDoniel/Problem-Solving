...
void solve(){
    ...
    map<int,int>mp,mpVintoKK;
    for(int i=0;i<n;i++){
        cin>>v[i];
        if(mp[v[i]%k]==0){
            mpVintoKK[v[i]%k]=kk.size();
            kk.pb({v[i]}); 
        }
        else{
            kk[mpVintoKK[v[i]%k]].pb(v[i]);
        }
        mp[v[i]%k]++;
    }
    ...
    // cout<<endl;
    int odd=0,oddElement;
    for(auto it:mp){
        if(it.second%2) odd++,oddElement=it.first;
    }
    if(n%2){
        if(odd==1){
            int ans=0;
            for(int i=0;i<kk.size();i++){
                ...
                if(kk[i].size()%2){
                    ...
                    for(int j=1;j<kk[i].size();j++){
                        int different=kk[i][j]-kk[i][j-1];
                        diff.pb(different);
                        if(j%2==0) oddPosSum+=different;
                    }
                    // cout<<"diff: ";
                    // print(diff);
                    // cout<<oddPosSum<<" "<<evenPosSum<<endl;
                    int sum=oddPosSum;
                    for(int j=1;j<diff.size();j+=2){
                        oddPosSum-=diff[j];
                        evenPosSum+=diff[j-1];
                        sum=min(sum,oddPosSum+evenPosSum);
                        // cout<<oddPosSum<<" "<<evenPosSum<<endl;
                    }
                    ans+=sum;
                }
                else{
                    for(int j=1;j<kk[i].size();j+=2){
                        ans+=kk[i][j]-kk[i][j-1];
                    }
                }
            }
