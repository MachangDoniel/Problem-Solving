...
void solve(){
    ...
    int n; cin>>n;
    vll a(n);
    mpl mp,mp2; // each element(box) contain 4 values (00,01,10,11 etc ...)
    vll index(n);   // index to box map
    for(int i=0;i<n;i++){
        cin>>a[i];
        mp[a[i]/4]++;   // how many number in a box
        index[i]=a[i]/4;
        mp2[a[i]]++;
    }
    for(int i=0;i<n;i++){
        int ind=index[i];
        for(int ele=4*ind;ele<4*ind+4;ele++){
            if(mp2[ele]){
                mp2[ele]--;
                cout<<ele<<" ";
                break;
