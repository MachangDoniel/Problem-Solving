...
// MyTask
// int mod=1e9+7;
int findAns(int k,int total,int total_max_subarray){
    // (total+total_max_subarray*power(2,k)-total_max_subarray)
    int ans=total_max_subarray%mod;
    int add=ans;
    for(int i=1;i<k;i++){
        add+=add;
        add%=mod;
    ...
    return (ans+total)%mod>=0?(ans+total)%mod:(ans+total)%mod+mod;
}
...
main()
{
    ...
    for(int t=1;t<=T;t++){
        // ll in,n,m,i,j,k,x,y;
        int n,k; cin>>n>>k;
        int a[n],total=0,total_max_subarray=0,curr_max_subarray=0;
        for(int i=0;i<n;i++){
            cin>>a[i];
            total+=a[i];
            curr_max_subarray=max(a[i],curr_max_subarray+a[i]); // we are including the current one, asking to include previous curr_max_subarray
            total_max_subarray=max(total_max_subarray,curr_max_subarray);
        }
        int ans=findAns(k,total,total_max_subarray);
        cout<<ans<<endl;
