...
void solve(int &t,int &T){
    ...
    for(int i=0;i<n;i++){
        int x,y; cin>>x>>y;
        hurdle[i].first=x;
        hurdle[i].second=y-x+2; // add 2 to cover entire hurdle
    }
    // sort(all(hurdle)); // already sorted
    vector<pair<int,int>>jump(m);
    for(int i=0;i<m;i++){
        cin>>jump[i].first>>jump[i].second;
    }
    // sort(all(jump)); // already sorted
    // reverse(all(jump));
    multiset<int>extra;
    int index=0,count=0,jumppower=1;
    for(int i=0;i<n;i++){
        int h=hurdle[i].first;
        int len=hurdle[i].second;
        for(;index<m && jump[index].first<h;index++){
            extra.insert(-jump[index].second);
        }
        bool flag=false;
        while(!extra.empty() && jumppower<len){
            int j=-*extra.begin();
