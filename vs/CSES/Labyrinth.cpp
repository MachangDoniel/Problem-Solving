...
int time_limit=1e5,time_count=0;
bool checkTime(){
    time_count++;
...
pll start(){
    while(1){
        ...
        if(isValid(i,j+1) && s[i][j+1]=='.') q.push({i,j+1}),s[i][j+1]='R';
        if(isValid(i,j-1) && s[i][j-1]=='.') q.push({i,j-1}),s[i][j-1]='L';
        if(isValid(i+1,j) && s[i+1][j]=='.') q.push({i+1,j}),s[i+1][j]='D';
        if(isValid(i-1,j) && s[i-1][j]=='.') q.push({i-1,j}),s[i-1][j]='U';
        
        if(isValid(i,j+1) && s[i][j+1]=='B'){
            s[i][j+1]='R';
            return {i,j+1};
        }
        else if(isValid(i,j-1) && s[i][j-1]=='B'){
            s[i][j-1]='L';
            return {i,j-1};
        }
        else if(isValid(i+1,j) && s[i+1][j]=='B'){
            s[i+1][j]='D';
            return {i+1,j};
        }
        else if(isValid(i-1,j) && s[i-1][j]=='B'){
