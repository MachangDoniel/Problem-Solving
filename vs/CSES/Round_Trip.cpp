...
void bfs(int node){
    ...
    while(!q.empty()){
        ...
        for(int i=0;i<adj[front].size();i++){
            // cout<<"    "<<adj[front][i]<<endl;
            if(!vis[adj[front][i]]){
                q.push(adj[front][i]);
                vis[adj[front][i]]=true;
                parent[adj[front][i]]=front;
            }
            if(parent[front]==adj[front][i]) continue;
            // cout<<"parent: "<<parent[adj[front][i]]<<"/"<<front<<", child: "<<adj[front][i]<<endl;
            // cout<<"check: "<<adj[front][i]<<" "<<parent[adj[front][i]]<<" "<<front<<endl;
            if(parent[adj[front][i]]!=-1 && parent[adj[front][i]]!=front){
                int newparent=front,oldparent=parent[adj[front][i]];
                int child=adj[front][i];
                // cout<<"found: "<<child<<" "<<oldparent<<" "<<newparent<<endl;
                mpl mp;
                stack<int>st;
                queue<int>qq;
                st.push(child);
...
void solve(){
    ...
    // for(int i=1;i<=n;i++) cout<<i<<": ",print(adj[i]);
    // cout<<endl;
    for(int i=1;i<=n;i++){
        if(flag) return;
        if(!vis[i]) bfs(i);
