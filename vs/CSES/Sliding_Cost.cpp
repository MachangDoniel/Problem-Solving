...
int main()
{
    ...
    while(T--){
        ...
        ordered_set<pair<ll,ll>>orderedSet;
        for(i=0;i<k;i++){
            orderedSet.insert({v[i],i});
        }
        ll old_mid=orderedSet.find_by_order((k-1)/2)->first,ans=0;
        for(i=0;i<k;i++){
            ans+=abs(old_mid-orderedSet.find_by_order(i)->first);
        }
        cout<<ans<<" ";
        for(i=0;i<n-k;i++){
            orderedSet.erase({v[i],i});
            orderedSet.insert({v[k+i],k+i});
            ll mid=orderedSet.find_by_order((k-1)/2)->first;
            ans+=abs(mid-v[k+i])-abs(old_mid-v[i]);
            if(k%2==0) ans+=(old_mid-mid);
            old_mid=mid;
            cout<<ans<<" ";
        }
        cout<<endl;
