...
int n;
vector<int>v;
bool expandableRightForMin(int index, int mx) {
    return index < n - 1 && mx > v[index + 1];
}
bool expandableLeftForMin(int index, int mx) {
    return index > 0 && mx > v[index - 1];
}
bool expandableRightForMax(int index, int mn) {
    return index < n - 1 && mn < v[index + 1];
}
bool expandableLeftForMax(int index, int mn) {
    return index > 0 && mn < v[index - 1];
}
...
void solve(int &t,int &T){
    ...
    cin>>n;
    v.resize(n);
    int mx=minf,mn=inf,minIndex,maxIndex;
    for(int i=0;i<n;i++){
        cin>>v[i];
        if(v[i]>mx){
            mx=v[i];
    ...
    {
        multiset<int>ms;
        multiset<int>ms1(all(v));
        ms.insert(v[minIndex]);
        ms1.erase(ms1.find(v[minIndex]));
        int right=minIndex,left=minIndex;
        while(expandableRightForMin(right,mx) || expandableLeftForMin(left,mx)){
            if(expandableRightForMin(right,mx) && expandableLeftForMin(left,mx)){
                if(v[right+1]<v[left-1]){
                    right++;
                    ms.insert(v[right]);
                    ms1.erase(ms1.find(v[right]));
                }
                else{
                    left--;
                    ms.insert(v[left]);
                    ms1.erase(ms1.find(v[left]));
                }
            }
            else if(expandableRightForMin(right,mx)){
                right++;
                ms.insert(v[right]);
                ms1.erase(ms1.find(v[right]));
            }
            else if(expandableLeftForMin(left,mx)){
                left--;
                ms.insert(v[left]);
                ms1.erase(ms1.find(v[left]));
            }
            ...
            // print(ms);
            // print(ms1);
            minimum = min(minimum, mx - min(*ms1.begin(), lowest + mx - highest));
            // cout<<minimum<<endl;
        }
    }
    // cout<<"Min: ";
    ...
    {
        multiset<int>ms;
        multiset<int>ms2(all(v));
        ms.insert(v[maxIndex]);
        ms2.erase(ms2.find(v[maxIndex]));
        int right=maxIndex,left=maxIndex;
        while(expandableRightForMax(right,mn) || expandableLeftForMax(left,mn)){
            if(expandableRightForMax(right,mn) && expandableLeftForMax(left,mn)){
                if(v[right+1]>v[left-1]){
                    right++;
                    ms.insert(v[right]);
                    ms2.erase(ms2.find(v[right]));
                }
                else{
                    left--;
                    ms.insert(v[left]);
                    ms2.erase(ms2.find(v[left]));
                }
            }
            else if(expandableRightForMax(right,mn)){
                right++;
                ms.insert(v[right]);
                ms2.erase(ms2.find(v[right]));
            }
            else if(expandableLeftForMax(left,mn)){
                left--;
                ms.insert(v[left]);
                ms2.erase(ms2.find(v[left]));
            }
            ...
            int highest=*ms.rbegin();
            int lowest=*ms.begin();
            minimum = min(minimum, max(*ms2.rbegin(), highest - (lowest - mn)) - mn);
            // cout<<minimum<<endl;
        }
    }
    // cout<<"Min: ";
