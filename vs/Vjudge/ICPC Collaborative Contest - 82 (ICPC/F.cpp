...
int firstDayOfYear(int year) {
    int d = 4; // January 1, 1970 was a Thursday (4)
    for (int y = 1970; y < year; ++y) {
        d = (d + (isLeapYear(y) ? 2 : 1)) % 7;
    }
    // cout<<d<<endl;
    return d;
}

int getCalendarRows(int year) {
    int total = 0;
    int monthsfirstDay = firstDayOfYear(year); 

    for (int m = 1; m <= 12; m++) {
        int d = daysInMonth(year, m); 
        int firstWeekDays = (monthsfirstDay == 0) ? 7 : (7 - monthsfirstDay); 

        if (firstWeekDays > d) {
            firstWeekDays = d; 
        }
        
        int remainingDays = d - firstWeekDays;
        int rows = 1; 
        if (remainingDays > 0) {
        ...
        total += rows; 
        monthsfirstDay = (monthsfirstDay + d) % 7; 
    }
...
void printCalendar(int year) {
    ...
    for (int m = 1; m <= 12; m++) {
        cout << "\n--------- " << (m == 1 ? "January" : m == 2 ? "February" : m == 3 ? "March" :
                                 ...
                                 m == 10 ? "October" : m == 11 ? "November" : "December") << " " << year << " ---------\n";
        
        cout << "Sun Mon Tue Wed Thu Fri Sat\n";

        for (int i = 0; i < monthsfirstDay; i++) {
            cout << "    "; 
        }
