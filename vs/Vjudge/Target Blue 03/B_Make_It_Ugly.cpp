...
void solve(){
    ...
    for(int i=0;i<n;i++){
        cin>>a[i];
        if(i && a[i]!=a[i-1]) allsame=false;
    }
    // if allsame-> it can't be make non-beautiful
    // as the array is beautiful, first and last value is always same
    // the min consecutive of the first element is the ans
    int ans=inf,count=0;
    for(int i=0;i<n;i++){
        if(a[0]==a[i]) count++;
        else ans=min(ans,count),count=0;
    }
    ans=min(ans,count);
    allsame?cout<<-1<<endl:cout<<ans<<endl;
    // print(a);
}
