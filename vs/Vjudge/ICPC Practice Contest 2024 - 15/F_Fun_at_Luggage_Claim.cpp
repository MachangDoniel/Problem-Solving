...
void solve(int &t,int &T){
    ...
    set<pair<int,int>>increasable;
    for(int i=0;i<n;i++){
        int prevIndex=(i+n-1)%n;
        int nextIndex=(i+1)%n;
        if(a[i]-2>=a[prevIndex] && a[i]-2>=a[nextIndex]) increasable.insert({a[i],i});
    }
    for(int i=0;i<n;i++){
        cin>>b[i];
    }
    dbg(increasable);
    for(auto it:increasable){
        int value=it.first, index=it.second;
        int prevIndex=(index+n-1)%n;
        int nextIndex=(index+1)%n;
        int remove=value-b[index];
        if(remove>0){
            a[index]=b[index];
            a[prevIndex]+=remove/2;
            a[nextIndex]+=remove/2;
            if(!(a[index]+1>=a[prevIndex] && a[index]+1>=a[nextIndex])){
                cout<<"No"<<endl;
                return;
            }
        }
