...
bool isC(const vector<int>& dump){
    for(int i=0;i<dump.size();i++){
        if(dump[i]==0x00){  // null byte
            for(int j=0;j<i;j++){
                if(dump[j]<0x20 || dump[j]>0x7f) return false;  // Invalid as before before null byte
            }
            return true;
        }
    }
...
bool isP(vector<int>& dump){
    if(dump.empty()) return false;  // Empty dump
    int length = dump[0]; 
    if(dump.size()<(length+1)) return false;  // Not enough bytes
...
vector<int> makeDumps(string input) {
    vector<int> dump;
    stringstream ss(input);
    string byte;
    while(ss>>byte){
        int value=0; 
        for (char hexDigit:byte) { 
            value*=16; 
...
void solve(int &t,int &T){
    ...
    vector<int>dump=makeDumps(input);
    bool c=isC(dump);
    bool p=isP(dump);
    if(c && p) cout<<"Both"<<endl;
    else if(c) cout<<"C"<<endl;
    else if(p) cout<<"Pascal"<<endl;
    else cout<<"None"<<endl;
}
