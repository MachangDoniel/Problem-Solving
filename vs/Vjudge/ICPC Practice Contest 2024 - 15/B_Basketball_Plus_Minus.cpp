...
void replace(string team,string player1,string player2){
    if(team==team1){
        for(int i=0;i<5;i++){
            if(one[i]==player1){
                dbg(player2);
                dbg(t1);
                if(t1.find(player2)==t1.end()){
                    t1.insert({player2,0});
                    players.pb({team,player2});
                }
                one[i]=player2;
                break;
            }
...
void increase(string team,int point){
    if(team==team1){
        for(int i=0;i<5;i++){
            t1[one[i]]+=point;
            t2[two[i]]-=point;
...
void print(){
    ...
    for(int i=0;i<players.size();i++){
        string team=players[i].first;
        string player=players[i].second;
        int point;
        if(team==team1){
            cout<<player<<" ("<<team<<") ";
            point=t1[player];
...
void solve(int &t,int &T){
    ...
    int q; cin>>q;
    for(int i=0;i<q;i++){
        string hudai,team,action; cin>>hudai>>team>>action;
        if(action=="scored"){
            int point; cin>>point;
            increase(team,point);
        }
        else if(action=="replaced"){
            string player1, hudai, player2; cin>>player1>>hudai>>player2;
            replace(team,player1,player2);
        }
        dbg(t1);
        dbg(t2);
    }
