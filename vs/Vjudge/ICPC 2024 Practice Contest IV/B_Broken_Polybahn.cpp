...
void dfs(int node, int parent, vector<vector<int>>& graph, vector<int>& degrees) {
...
void solve() {
    ...
    dfs(0, -1, graph, degrees); 

    long long total = 0;
    for (int deg : degrees) {
        total = (total + power(2, deg, mod)) % mod; 
    }

    total = (total - (2LL * n) % mod + mod) % mod; 
    total = (total + 1) % mod; 
