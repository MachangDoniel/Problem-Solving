...
void ans(int mul12,int mul13,int mul45,int mul46){
    vector<int>v={42,23,16,15,8,4};
    map<int,int>mp;
    mp[42]=mp[23]=mp[16]=mp[15]=mp[8]=mp[4]=1;
    // for 1,2,3
    for(int num : v){
        if(mul12%num==0 && mp[mul12/num]==1 && mul13%num==0 && mp[mul13/num]==1 && num!=mul12/num && num!=mul13/num){
            cout<<"! "<<num<<" "<<mul12/num<<" "<<mul13/num<<" ";
            mp[num]=2;
            mp[mul12/num]=2;
            mp[mul13/num]=2;
            break;
    ...
    // for 4,5,6
    for(int num : v){
        if(mp[num]==2) continue;
        if(mul45%num==0 && mp[mul45/num]==1 && mul46%num==0 && mp[mul46/num]==1 && num!=mul45/num && num!=mul46/num){
            cout<<num<<" "<<mul45/num<<" "<<mul46/num<<endl;
            break;
        }
    }
