...
#define leftSuml set<ll>
#define mleftSum multiset<ll>
#define oleftSum ordered_set<ll>
#define moleftSum multi_ordered_set<ll>
...
void solve(int &t,int &T){
	...
	int n; cin>>n;
    vector<int>v(n);
    vector<pair<int,int>>rightsplitSum(n),leftsplitSum(n);
	...
	for(int i=0;i<n;i++){
		rightSum+=v[i];
		while(index<=i && abs(leftSum-rightSum)>=abs(leftSum+v[index]-(rightSum-v[index]))){
            leftSum += v[index];
            rightSum-=v[index];
            index++;
        }
		leftsplitSum[i]={leftSum, rightSum};
	}
	leftSum=0;
	rightSum=0;
	index=n-1;
	for (int i=n-1;i>=0;i--) {
		...
		rightsplitSum[i]={leftSum, rightSum};
	}
	int ans=inf;
	for (int i=1;i<n-2;i++) {
		int p=leftsplitSum[i].first;
		int q=leftsplitSum[i].second;
		int r=rightsplitSum[i+1].first;
		int s=rightsplitSum[i+1].second;
