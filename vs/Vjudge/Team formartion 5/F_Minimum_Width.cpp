...
bool canFit(const vector<ll>& lengths, ll W, ll M) {
    ll currentWidth = 0;
    ll lines = 1;
    for (ll i = 0; i < lengths.size(); ++i) {
        if (currentWidth + lengths[i] + (currentWidth > 0 ? 1 : 0) <= W) {
            currentWidth += lengths[i] + (currentWidth > 0 ? 1 : 0);
        } else {
            ++lines;
...
void solve() {
    ...
    ll low = maxLength;
    ll high = sumLengths + (N - 1);

    while (low < high) {
        ll mid = low + (high - low) / 2;
        if (canFit(lengths, mid, M)) {
            high = mid;
        } else {
            low = mid + 1;
        }
