...
void solve() {
    ...
    if(B*B-4*A*C>=0){
        ll com=sqrt(B*B-4*A*C);
        ll x1=min((-B-com)/(2*A),(-B+com)/(2*A));
        ll x2=max((-B-com)/(2*A),(-B+com)/(2*A));
        ll k1,k2,down;
        if(B){
            k1=2*A;
            k2=3*B;
        ...
        auto f=[&](ll x){
            return k1*x*x*x+k2*x*x+C*x*down;
        };
        ll ansUP=0;
        if(L<=x1 && x1<=R){
            ll area=abs(f(L)-f(x1));
            ansUP+=area;
        }
        if(L<=x2 && x2<=R){
            ll area=abs(f(R)-f(x2));
            ansUP+=area;
        }
        ll area=abs(f(max(x1,L))-f(min(x2,R)));
        ansUP+=area;
        ll g=gcd(ansUP,down);
            cout<<ansUP/g<<"/"<<down/g<<endl;
