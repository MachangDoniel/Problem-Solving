...
int getDivisorSum(int n){
    ...
    for(int i=1;i*i<=n;i++){
        ...
        if(n%i==0){
            ...
            if(i!=n/i && n/i!=n) div.push_back(n/i);
        }
    }
    int sum=0;
...
void solve() {
    ...
    // cout<<" "<<sum<<" "<<getDivisorSum(sum)<<endl;
    bool comp=true;
    if(n==sum) cout<<" perfecto",comp=false;
    if(n!=sum && n==getDivisorSum(sum)) cout<<" romantico",comp=false;
    if(n<sum) cout<<" abundante",comp=false;
    if(comp) cout<<" complicado";
    cout<<endl;
}
