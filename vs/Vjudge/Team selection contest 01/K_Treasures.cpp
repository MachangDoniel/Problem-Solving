...
int eat(int row,int col){
    return num(s[row][col]);
}
bool risk(int row,int col){
    if((valid(row-1,col) && s[row-1][col]=='T')) return true;
    if((valid(row+1,col) && s[row+1][col]=='T')) return true;
    if((valid(row,col-1) && s[row][col-1]=='T')) return true;
    if((valid(row,col+1) && s[row][col+1]=='T')) return true;
    return false;
}
...
int ans=0;
void find_graph(int row,int col){
    // cout<<row<<" "<<col<<" "<<n<<" "<<m<<endl;
    mp[{row,col}]++;
    ans+=eat(row,col);
    if(risk(row,col)) return;
    if(s[row][col]!='#'){
        if(valid(row-1,col) and !mp[{row-1,col}]) find_graph(row-1,col);
        if(valid(row+1,col) and !mp[{row+1,col}]) find_graph(row+1,col);
        if(valid(row,col-1) and !mp[{row,col-1}]) find_graph(row,col-1);
        if(valid(row,col+1) and !mp[{row,col+1}]) find_graph(row,col+1);
    }
    return;
}
...
void solve() {
    ...
    while(cin>>n>>m){
        ...
        mp.clear();
        ans=0;
        mp[{start_row,start_col}]++;
        find_graph(start_row,start_col);
        cout<<ans<<endl;
    }
}
