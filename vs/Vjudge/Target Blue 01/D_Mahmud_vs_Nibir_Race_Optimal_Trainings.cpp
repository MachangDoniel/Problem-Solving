...
bool good(int l,int r,int u){
    return pre[r]-pre[l-1]>=u;
}
...
main()
{
    ...
    for(int t=1;t<=T;t++){
        ...
        // print(pre);
        for(int i=0;i<q;i++){
            int l,r,u; cin>>l>>u;   // pre[r]-pre[l-1]
            int low=l-2,high=n+1;
            while(high>low+1){
                int mid=(high+low)/2;
                if(good(l,mid,u)) high=mid;
                else low=mid;
            }
            
            int performanceAtHigh=u*(u+1)/2-(pre[high]-pre[l-1]-u-1)*(pre[high]-pre[l-1]-u)/2;
            int performanceBeforeHigh=u*(u+1)/2-(pre[high-1]-pre[l-1]-u-1)*(pre[high-1]-pre[l-1]-u)/2;
            // cout<<high<<" "<<performanceAtHigh<<" "<<performanceBeforeHigh<<endl;
            if(performanceAtHigh>performanceBeforeHigh) ans[i]=min(n,high);
            else ans[i]=max(l,high-1);
            // cout<<pre[high]<<" "<<pre[l-1]<<" "<<u<<" "<<u*(u+1)/2<<" "<<(pre[high]-pre[l-1]-u-1)*(pre[high]-pre[l-1]-u)/2<<endl;
