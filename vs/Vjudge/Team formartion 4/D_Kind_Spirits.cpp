...
vector<vector<pair<int,int>>>solar_sytem(1000);     // at most 1000>(N=30*30=900) planets in solar system
vector<int>costs(1000,inf);
bool vis[1000]={false};
void bfs(int node){
    queue<int>q;
    q.push(node);
    ...
    while(!q.empty()){
        int current=q.front();
        q.pop();
        for(int i=0;i<solar_sytem[current].size();i++){
            if(!vis[solar_sytem[current][i].first]){
                q.push(solar_sytem[current][i].first);
                vis[solar_sytem[current][i].first]=true;
            }
            costs[solar_sytem[current][i].first]=min(costs[solar_sytem[current][i].first],costs[current]+solar_sytem[current][i].second);
        }
    }
}
...
void solve(){
    // ll in,n,m,i,j,k,x,y;
    // int n; cin>>n;
    int no_of_level; cin>>no_of_level;
    int countplanet=1;
    vll heaven,prev;
    prev.pb(countplanet);
    for(int i=1;i<=no_of_level;i++){
        int planets; cin>>planets; // at i level
        vll current(planets);
        for(int j=0;j<planets;j++){
            current[j]=++countplanet;
        }
        for(int j=0;j<planets;j++){
            while(1){
                int from; cin>>from;
                if(from==0) break;
                int cost; cin>>cost;
                solar_sytem[prev[from-1]].pb({current[j],cost});
            }
        }
        string s; cin>>s;
        prev=current;
        if(i==no_of_level) heaven=current;
    }
    // for(int i=0;i<countplanet;i++){
    //     cout<<"from "<<i<<":"<<endl;
    //     for(int j=0;j<solar_sytem[i].size();j++){
    //         cout<<" "<<solar_sytem[i][j].first<<" "<<solar_sytem[i][j].second<<endl;
    //     }
    // }
    bfs(1);
    int ans=inf;
