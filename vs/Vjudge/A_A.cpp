...
main()
{
    ...
    for(int t=1;t<=T;t++){
        ...
        int right=v[rightIndex],left=v[leftIndex],count=0;
        while(checkTime){
            if(rightIndex<=leftIndex){
                break;
            }
            // cout<<left<<" "<<right<<" "<<rightIndex<<" "<<leftIndex<<endl;
            if(right==left) rightIndex--,leftIndex++,right+=v[rightIndex],left+=v[leftIndex];
            else if(right<left) count++,rightIndex--,right+=v[rightIndex];
            else count++,leftIndex++,left+=v[leftIndex];
