...
class Baradah {
    private:
        int parent[N],no_of_fish[N];
        multiset<int>maxFish;
    public:
        Baradah() {
            // for(int i=0;i<N;i++) {
            //     parent[i]=i;
        ...
        void make_section(int v,int fish) {
            parent[v]=v;
            no_of_fish[v]=fish;
            maxFish.insert(fish);
        }
        ...
        void union_sets(int current,int next) {
            current=find_set(current);
            next=find_set(next);
            if(current!=next){

                maxFish.erase(maxFish.find(no_of_fish[current]));
                // cout<<current<<" "<<next<<": "<<endl;
                
                if(next<n) {
                    maxFish.erase(maxFish.find(no_of_fish[next]));
                    no_of_fish[next]+=no_of_fish[current];
                    maxFish.insert(no_of_fish[next]);
                }
                no_of_fish[current]=0;
                parent[current]=next;
                maxFish.insert(no_of_fish[current]);
            }
            showMax();
