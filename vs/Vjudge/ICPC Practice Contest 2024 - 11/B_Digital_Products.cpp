...
vector<pair<int, int>> v;  // Stores (number, product of digits)
map<int, int> mp;  // Map to keep track of products to avoid duplicates



// Function to precompute all the products of digits and store them
void make() {
    for (int i = 1; i <= 9; i++) {
        v.push_back({i, i});  // For single digit numbers, product is the number itself
        mp[i] = 1;
    }
    int mul = 1;
    for (int i = 1; i <= 18; i++) {  // Consider numbers up to 18 digits
        mul *= 10;
        int n = v.size();
        for (int j = 1; j <= 9; j++) {  // Adding products for each digit
            for (int k = 0; k < n; k++) {
                int newProduct = j * v[k].second;  // Calculate new product
                if (mp[newProduct] == 0) {
                    v.push_back({mul * j + v[k].first, newProduct});  // Store the number and its product
                    mp[newProduct] = 1;
                }
            }
        }
...
bool good(int mid,int n){
    return v[mid].first>=n;
}

void solve(int &t,int &T){
    // ll in,n,m,i,j,k,x,y;
    int n; cin>>n;
    int high=v.size(),low=-1;
    while(high>low+1){
        int mid=(high+low)/2;
        // cout<<"High: "<<high<<" Low: "<<low<<" Mid: "<<mid<<" "<<v[mid].first<<endl;
