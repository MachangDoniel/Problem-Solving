...
{
    int x,y,z; cin>>x>>y>>z;
    int s=(x+y+z); // need to divied by 2
    int upor=(s-2*x)*(s-2*y)*(s-2*z); // need to divide by 2
    map<int,int>up=getPrimes(upor);
    map<int,int>down=getPrimes(4*s);
    for(auto it:down){
        int x=it.first;
        if(up[x]>=down[x]){
            up[x]-=down[x];
    ...
    int upp=1,downn=1;
    for(auto it:up){
        upp*=power(it.first,it.second);
    }
    for(auto it:down){
        downn*=power(it.first,it.second);
    }
    cout<<upp<<"/"<<downn<<endl;
}
