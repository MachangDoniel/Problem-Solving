...
bool canAchieveBit(vector<long long>&v,int n,int m,int k,ll bit) {
    vector<ll> increments;
    for (int i=0;i<n;i++) {
        if ((v[i] & bit)==0) {
            ll increment=bit-(v[i] & (bit-1));
            increments.push_back(increment);
        }
    }
    sort(increments.begin(), increments.end());

    ll totalIncrements = 0;
    int count=0;
    for (long long inc : increments) {
        if (totalIncrements + inc <= m) {
            totalIncrements += inc;
            count++;
        } else {
            break;
...
void solve() {
    ...
    for (int bitPos=30;bitPos>=0;bitPos--) {
        ll bit=1LL<<bitPos;
        if (canAchieveBit(v,n,m,k, result | bit)) {
            result|=bit;
        }
    }
    cout<<result<<endl;
