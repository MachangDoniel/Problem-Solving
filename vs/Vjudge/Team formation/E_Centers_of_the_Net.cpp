...
void dfs(int x){
    ...
    for(int i=0;i<adj[x].size();i++){
        if(!visited[adj[x][i]]){
            disfromroot[adj[x][i]]=disfromroot[x]+1;
            dfs(adj[x][i]);
        }
    }
}
...
void solve(){
    ...
    // }
    dfs(1);
    int maxdisfromroot=0,maxnode=1;
    for(int i=1;i<n+1;i++){
        if(maxdisfromroot<disfromroot[i]){
            maxdisfromroot=max(maxdisfromroot,disfromroot[i]);
            maxnode=i;
        }
        disfromroot[i]=0;
    }
    ...
    int maxdis=0;
    for(int i=1;i<n+1;i++){
        maxdis=max(maxdis,disfromroot[i]);
        disfromroot[i]=0;
    }
    for(int i=1;i<=n;i++){
