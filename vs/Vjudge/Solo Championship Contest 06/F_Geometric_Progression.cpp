...
struct SimpleHash {
    ...
    inline ll range_hash(ll l, ll r) {
        ll hashval = (H[r + 1] - (P[r - l + 1] * H[l] % mod)) % mod;
        return (hashval < 0 ? hashval + mod : hashval);
    }
    inline ll reverse_hash(ll l, ll r) {
        ll hashval = R[l + 1] - (P[r - l + 1] * R[r + 2] % mod);
        return (hashval < 0 ? hashval + mod : hashval);
    }
};
struct DoubleHash {
    ...
    ll concate(DoubleHash& B, ll l1, ll r1, ll l2, ll r2) {
        // ll len1 = r1 - l1 + 1, len2 = r2 - l2 + 1; // Removed len1 to avoid unused variable warning
        ll len2 = r2 - l2 + 1; // Keeping len2 as it is used
        ll x1 = sh1.range_hash(l1, r1),
                  x2 = B.sh1.range_hash(l2, r2);
        x1 = (x1 * B.sh1.P[len2]) % 2091573227;
        ll newx1 = (x1 + x2) % 2091573227;
        x1 = sh2.range_hash(l1, r1);
        x2 = B.sh2.range_hash(l2, r2);
        x1 = (x1 * B.sh2.P[len2]) % 2117566807;
        ll newx2 = (x1 + x2) % 2117566807;
...
ll geometric_sum(ll a,ll x,ll m) {
    if (a==1) {
        return x%m;
    }
    ll A_exp_X=mod_exp(a,x,m);
    ll numerator=(A_exp_X-1+m)%m;
    ll denominator_inv=mod_inverse(a-1,m);
